"""
AR Fine-tuning: Multi-step Autoregressive Training.

Requires pretrained model from train_pretrain_v3.py (Teacher Forcing).

Usage:
    CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 torchrun --nproc_per_node=8 train_ar.py --config configs/ar.yaml --pretrained checkpoints_v3/best_tf.pt
"""
...
def compute_multi_step_loss(
    ...
    required_T = t_input + num_steps
    assert T_total >= required_T, \
        f"Data has {T_total} timesteps, but need {required_T} for AR"
    ...
    for step in range(1, num_steps):
        ...
        gt_part = data[:, step:step + t_input - 1]
        pred_part = pred_denorm[:, -1:]
        new_input = torch.cat([gt_part, pred_part], dim=1)

        target_data = data[:, step + 1:step + 1 + t_input]
...
def parse_args():
    parser = argparse.ArgumentParser(description="PDE Foundation Model - AR Fine-tuning")
    parser.add_argument('--config', type=str, required=True, help='Path to config file')
    parser.add_argument('--pretrained', type=str, required=True, help='Path to pretrained TF checkpoint (REQUIRED)')
    parser.add_argument('--resume', type=str, default=None, help='Path to AR checkpoint to resume from')
...
def load_pretrained_weights(model, pretrained_path: str, accelerator):
    """Load pretrained weights from TF checkpoint. This is REQUIRED."""
    if not pretrained_path:
        raise ValueError("--pretrained is required for AR training. Run train_pretrain_v3.py first.")
...
def load_ar_checkpoint(model, optimizer, scheduler, checkpoint_path: str, accelerator):
    """Load AR checkpoint for resuming."""
    if not checkpoint_path or not Path(checkpoint_path).exists():
        return 0, float('inf')

    if accelerator.is_main_process:
        logger.info(f"Resuming AR training from: {checkpoint_path}")
...
def main():
    ...
    model_name = config.get('model_name', 'pde_ar')
    ...
    # Create dataloaders (need t_input + num_steps timesteps for AR)
    ...
    temporal_length = t_input + num_steps  # AR needs more timesteps
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info(f"PDE Foundation Model - AR Fine-tuning")
        logger.info(f"{'='*60}")
        ...
        logger.info(f"T_input: {t_input}, Num Steps: {num_steps}, Lambda: {lambda_decay}")
        logger.info(f"Temporal Length: {temporal_length}")
        logger.info(f"Resume from: {args.resume or 'N/A'}")
        logger.info(f"{'='*60}")
    ...
    # Create optimizer (usually lower LR for fine-tuning)
    ...
    # Load pretrained weights (REQUIRED)
    load_pretrained_weights(model, args.pretrained, accelerator)

    # Resume AR training if checkpoint provided
    global_step, best_val_rmse = 0, float('inf')
    if args.resume:
        global_step, best_val_rmse = load_ar_checkpoint(
            model, optimizer, scheduler, args.resume, accelerator
        )

    if accelerator.is_main_process:
        run_name = f"ar-{model_name}-h{hidden_dim}-L{num_layers}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["AR", f"h{hidden_dim}", f"L{num_layers}", f"steps{num_steps}"],
            }}
        )
    ...
    with Progress(
        ...
        train_task = progress.add_task("AR Training", total=total_steps, completed=global_step)
        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch in train_loader:
                ...
                phase_str = "[warmup]" if in_warmup else "[AR]"
                progress.update(
                    ...
                    if not in_warmup:
                        if val_rmse < best_val_rmse:
                            ...
                            save_checkpoint(
                                ...
                                config, save_dir, accelerator, 'best_ar.pt'
                            )
                            if accelerator.is_main_process:
                                console.print(f"[yellow]Saved best AR model[/yellow] (val_rmse: {val_rmse:.6f})")
                        else:
                            patience_counter += 1
                            if accelerator.is_main_process:
    ...
    if accelerator.is_main_process:
        table = Table(title="AR Training Complete", show_header=False, border_style="green")
        ...
        table.add_row("Checkpoint", str(save_dir / "best_ar.pt"))
        console.print(table)
