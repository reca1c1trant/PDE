"""
LoRA Finetuning for POSEIDON NS-SVS (Incompressible NS + Tracer, periodic BC).

Loss: BC loss + PDE loss (NO RMSE)
- BC loss: RMSE on boundary pixels (supervised anchor for periodic domain)
- PDE loss: Divergence-free + Vorticity transport + Tracer transport
  Per-equation normalized by GT RMS scale -> all equations ~O(1)

Same PDE as NS-PwC, only different IC (sinusoidal vortex sheet).
Reuses NSPwCPDELoss directly.

Channel mapping (18-channel):
- Channel 0 = Vx, Channel 1 = Vy (vector_dim=2)
- Channel 14 = scalar[11] = passive_tracer (scalar_indices=[11])

No force field, no pressure, periodic BC.

Usage:
    torchrun --nproc_per_node=8 finetune/train_ns_svs_lora_v3.py --config configs/finetune_ns_svs_v3.yaml
"""
...
# Channel indices in 18-channel layout (same as NS-PwC / shear_flow)
CH_VX = 0       # vector: Vx
CH_VY = 1       # vector: Vy
CH_TRACER = 14  # scalar[11] = passive_tracer (scalar_indices=[11])


def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for NS-SVS (V3 Model)")
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("NS-SVS LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info("Loss: BC (boundary supervised) + PDE (div + vorticity + tracer)")
    ...
    # Create PDE loss function (reuse NSPwCPDELoss — same PDE equations)
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"ns_svs-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["ns_svs", "lora", "v3", "periodic", "BC+PDE", "normalized"],
            }}
        )
