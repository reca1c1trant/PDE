"""
LoRA Finetuning for 1D KdV (Korteweg-de Vries) using PDEModelV3.

Loss: BC loss + PDE loss (NO RMSE)
- BC loss: RMSE on boundary pixels (supervised anchor for periodic domain)
- PDE loss: u_t - beta_1*u*u_x - alpha_3*u_xxx - alpha_4*u_xxxx = 0

Channel mapping (18-channel):
- Channel 3 = scalar[0] = u (scalar_indices=[0])
- vector_dim=0

BCs: Periodic in x
No per-sample parameter variation (all samples share same PDE coefficients)
...
Usage:
    torchrun --nproc_per_node=4 finetune/train_kdv_1d_lora_v3.py \
        --config configs/finetune_kdv_1d_v3.yaml
"""
...
from finetune.pde_loss_verified import KdV1DPDELoss

logger = logging.getLogger(__name__)

# KdV channel index: u -> scalar[0] = channel 3 (3 vector + 0 scalar offset)
CH_U = 3


def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for 1D KdV (V3 Model)")
...
def compute_pde_loss(
    ...
    Args:
        ...
        pde_loss_fn: KdV1DPDELoss instance
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("1D KdV LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Loss: BC (w={bc_width}) + PDE (KdV: convection + dispersion + hyp_diff)")
    ...
    pde_loss_fn = KdV1DPDELoss(
        ...
        alpha_3=physics.get('alpha_3', -8.5449e-7),
        alpha_4=physics.get('alpha_4', -1.7166e-9),
        beta_1=physics.get('beta_1', -1.2500e-2),
        eq_scales=eq_scales,
    ...
    if accelerator.is_main_process:
        logger.info(f"PDE coefficients: alpha_3={pde_loss_fn.alpha_3}, alpha_4={pde_loss_fn.alpha_4}, beta_1={pde_loss_fn.beta_1}")
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"kdv1d-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["kdv_1d", "lora", "v3", "BC+PDE", "normalized"],
            }}
        )
