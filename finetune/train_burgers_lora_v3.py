"""
LoRA Finetuning for 2D Burgers Equation using PDEModelV3.
...
Uses V3 Shared FFN Transformer with LoRA on:
- NA Attention: qkv, proj
- FFN: gate_proj, up_proj, down_proj

Usage:
    torchrun --nproc_per_node=8 finetune/train_burgers_lora_v3.py --config configs/finetune_burgers_v3.yaml
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for Burgers (V3 Model)")
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info(f"Burgers2D LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"burgers-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["burgers", "lora", "v3", "shared-ffn", "BC+PDE"],
            }}
        )
