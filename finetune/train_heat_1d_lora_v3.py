"""
LoRA Finetuning for 1D Heat Equation using PDEModelV3.

Loss: BC loss + PDE loss (NO RMSE)
- BC loss: RMSE on boundary pixels (supervised anchor for periodic domain)
- PDE loss: u_t - alpha*u_xx = 0, per-sample alpha from batch['nu']

Channel mapping (18-channel):
- Channel 17 = scalar[14] = temperature T
- vector_dim=0

BCs: Periodic in x
Per-sample alpha: from dataset nu field (stores thermal diffusivity)
...
Usage:
    torchrun --nproc_per_node=4 finetune/train_heat_1d_lora_v3.py \
        --config configs/finetune_heat_1d_v3_rescaled_norm.yaml
"""
...
from finetune.pde_loss_verified import Heat1DPDELoss

logger = logging.getLogger(__name__)

# 1D Heat channel index: T -> scalar[14] = channel 17 (3+14)
CH_T = 17


def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for 1D Heat Equation (V3 Model)")
...
def compute_pde_loss(
    ...
    Args:
        ...
        pde_loss_fn: Heat1DPDELoss instance
    ...
    with torch.autocast(device_type='cuda', enabled=False):
        t0_u = input_data[:, 0:1, :, CH_T].float()
        out_u = output[:, :, :, CH_T].float()

        # [B, T_out+1, X]
        u = torch.cat([t0_u, out_u], dim=1)

        total_loss, losses = pde_loss_fn(u, alpha=alpha.float())
...
def validate(
    ...
    for batch in val_loader:
        ...
        alpha = batch['nu'].to(device=accelerator.device)  # nu stores thermal diffusivity
        ...
        pde_loss, _ = compute_pde_loss(output, input_data, pde_loss_fn, alpha)
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("1D Heat Equation LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Loss: BC (w={bc_width}) + PDE (u_t - alpha*u_xx)")
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"heat1d-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["heat_1d", "lora", "v3", "BC+PDE", "normalized"],
            }}
        )
        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch in train_loader:
                ...
                with accelerator.accumulate(model):
                    ...
                    bc_loss = compute_boundary_loss(output, target_data, channel_mask, bc_width=bc_width)
                    pde_loss, losses = compute_pde_loss(output, input_data, pde_loss_fn, alpha)
