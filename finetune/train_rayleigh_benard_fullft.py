"""
Full-Param Finetuning for Rayleigh-Benard 2D (Incompressible Boussinesq).

Loss: BC loss + PDE loss (NO RMSE - that would be cheating!)
- BC loss: RMSE on boundary pixels (supervised anchor)
- PDE loss: Continuity (div u=0) + Buoyancy transport (db/dt + u*grad(b) = kappa*Lap(b))
...
- Channel 3 = scalar[0] = buoyancy
- Channel 15 = scalar[12] = pressure

BCs: x periodic, y Dirichlet (wall)
     b(y=0)~1, b(y=1)~0, u(wall)=0, v(wall)=0
Per-sample kappa: from dataset nu field (Pr=1 -> kappa=nu)
...
Usage:
    torchrun --nproc_per_node=8 finetune/train_rayleigh_benard_fullft.py --config configs/finetune_rayleigh_benard_v3.yaml
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Full-Param Finetuning for Rayleigh-Benard 2D")
...
def compute_pde_loss(
    ...
    Extracts u, v, buoyancy from denormalized model output,
    prepends t0 frame for time derivatives.
    kappa = nu (because Pr=1 for this dataset).
    """
    with torch.autocast(device_type='cuda', enabled=False):
        ...
        # Transpose spatial dims back for correct FD operators.
        u = u.transpose(-1, -2)
        v = v.transpose(-1, -2)
        b = b.transpose(-1, -2)
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info(f"Rayleigh-Benard 2D Full-Param Finetuning")
        logger.info(f"{'='*60}")
    ...
    if accelerator.is_main_process:
        run_name = "rayleigh_benard-fullft-v3"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["rayleigh_benard", "fullft", "v3", "BC+PDE", "normalized"],
            }}
        )
