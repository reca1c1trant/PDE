"""
LoRA Finetuning for 2D Advection-Diffusion using PDEModelV3.

Loss: BC loss + PDE loss (NO RMSE)
- BC loss: RMSE on boundary pixels (supervised anchor for periodic domain)
- PDE loss: Advection-diffusion equation residual, per-equation normalized

Channel mapping (18-channel):
- Channel 3 = scalar[0] = concentration/scalar u
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for Advection-Diffusion 2D (V3 Model)")
...
def compute_pde_loss(
    ...
    Args:
        ...
        pde_loss_fn: AdvDiff2DPDELoss instance
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("Advection-Diffusion 2D LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Loss: BC (boundary w={bc_width}) + PDE (advdiff)")
    ...
    if accelerator.is_main_process:
        ...
        logger.info(f"PDE params: a={pde_loss_fn.a}, b={pde_loss_fn.b}, nu={pde_loss_fn.nu}")
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"advdiff_2d-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["advdiff_2d", "lora", "v3", "periodic", "BC+PDE", "normalized"],
            }}
        )
        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch in train_loader:
                ...
                with accelerator.accumulate(model):
                    ...
                    # PDE loss (advection-diffusion, per-equation normalized)
