"""
From-Scratch Full-Parameter Training with BC + PDE Loss.

Used as ablation: same BC+PDE loss as LoRA finetune, but without pretrain weights.
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("FROM SCRATCH Full-Param Training (BC + PDE)")
        logger.info(f"{'='*60}")
    ...
    if accelerator.is_main_process:
        ...
        logger.info(f"Model parameters: {n_params:,} (all trainable, random init)")
