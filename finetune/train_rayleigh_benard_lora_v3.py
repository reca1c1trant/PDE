"""
LoRA Finetuning for Rayleigh-Bénard 2D (Incompressible Boussinesq).

Loss: BC loss + PDE loss (NO RMSE - that would be cheating!)
- BC loss: RMSE on boundary pixels (supervised anchor)
- PDE loss: Continuity (div u=0) + Buoyancy transport (∂b/∂t + u·∇b = κΔb)
...
Per-sample kappa (κ): from dataset nu field (Pr=1 → κ=ν)
...
Usage:
    torchrun --nproc_per_node=8 finetune/train_rayleigh_benard_lora_v3.py --config configs/finetune_rayleigh_benard_v3.yaml
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for Rayleigh-Bénard 2D")
...
def compute_patch_boundary_loss(
    ...
    Computes |f[P-2] - 2*f[P-1] + f[P]|² at each patch boundary.
...
def compute_pde_loss(
    ...
    κ = ν (because Pr=1 for this dataset).
    """
...
def main():
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"rayleigh_benard-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["rayleigh_benard", "lora", "v3", "BC+PDE", "normalized"],
            }}
        )
