"""
Full-Param Finetuning for Active Matter 2D.
...
Usage:
    torchrun --nproc_per_node=8 finetune/train_active_matter_fullft.py --config configs/finetune_active_matter_v3.yaml
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Full-Param Finetuning for Active Matter 2D")
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info(f"Active Matter 2D Full-Param Finetuning")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Loss: BC + PDE (normalized continuity + concentration + Dxx)")
    ...
    if accelerator.is_main_process:
        run_name = "active_matter-fullft-v3"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["active_matter", "fullft", "v3", "BC+PDE", "normalized"],
            }}
        )
