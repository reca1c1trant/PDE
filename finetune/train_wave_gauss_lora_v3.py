"""
LoRA Finetuning for Wave-Gauss Acoustic Wave Equation (absorbing BC).

Loss: BC loss + PDE loss (NO RMSE)
- BC loss: RMSE on boundary pixels (supervised anchor for absorbing boundary)
- PDE loss: Wave equation residual u_tt = c^2 * (u_xx + u_yy)
  Per-equation normalized by GT RMS scale -> all equations ~O(1)

Channel mapping (18-channel):
- Channel 3 = scalar[0] = displacement u (vector_dim=0)
- Channel 4 = scalar[1] = wave speed c (time-independent)

No velocity fields, absorbing BC.
...
Usage:
    torchrun --nproc_per_node=8 finetune/train_wave_gauss_lora_v3.py --config configs/finetune_wave_gauss_v3.yaml
"""
...
CH_U = 3    # scalar[0] = displacement u
CH_C = 4    # scalar[1] = wave speed c (time-independent)


def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for Wave-Gauss (V3 Model)")
...
def compute_boundary_loss(
    ...
    For absorbing BC domains this serves as a supervised anchor
    (provides ground truth signal on boundaries).
    """
...
def compute_pde_loss(
    ...
    """
    Compute Wave-Gauss PDE residual loss (wave equation).
    ...
    Args:
        ...
        pde_loss_fn: WaveGaussPDELoss instance
    ...
    with torch.autocast(device_type='cuda', enabled=False):
        ...
        # c is time-independent, take from any frame
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("Wave-Gauss LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info("Loss: BC (boundary supervised) + PDE (wave equation)")
    ...
    pde_loss_fn = WaveGaussPDELoss(
        ...
        dt=physics.get('dt', 0.071428571),
        skip_boundary=physics.get('skip_boundary', 2),
        eq_scales=eq_scales,
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"wave_gauss-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["wave_gauss", "lora", "v3", "absorbing", "BC+PDE", "normalized"],
            }}
        )
        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch in train_loader:
                ...
                with accelerator.accumulate(model):
                    ...
                    # BC loss (boundary pixels as supervised anchor)
                    bc_loss = compute_boundary_loss(output, target_data, channel_mask)

                    # PDE loss (wave equation, per-equation normalized)
                ...
                if global_step % eval_interval == 0:
                    ...
                    accelerator.log({
                        ...
                        'val/rmse': val_rmse,
                        'val/vrmse': val_vrmse,
                        'val/total_loss': lambda_bc * val_bc + lambda_pde * val_pde,
                    }, step=global_step)
                    ...
                    if accelerator.is_main_process:
                        console.print(
                            ...
                            f"val_rmse={val_rmse:.6f}, val_vrmse={val_vrmse:.6f}"
                        )
