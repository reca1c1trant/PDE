"""
Full-Param Finetuning for Shear Flow (Incompressible NS + Tracer) using PDEModelV3.

Loss: BC loss + PDE loss (NO RMSE)
- BC loss: RMSE on boundary pixels (supervised anchor for periodic domain)
- PDE loss: Continuity + Momentum (u,v) + Tracer transport, per-equation normalized
...
Usage:
    torchrun --nproc_per_node=8 finetune/train_shear_flow_fullft.py --config configs/finetune_shear_flow_v3.yaml
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Full-Param Finetuning for Shear Flow (V3 Model)")
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("Shear Flow Full-Param Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info("Loss: BC (boundary supervised) + PDE (cont + u_mom + v_mom + tracer)")
        logger.info(f"Lambda BC: {lambda_bc}, PDE: {lambda_pde}")
        logger.info(f"Save: {save_rmse_weight}*rmse + {save_pde_weight}*pde")
        logger.info(f"{'='*60}")
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["shear_flow", "fullft", "v3", "periodic", "BC+PDE", "normalized"],
            }}
        )
                        ...
                        if save_metric < best_val_loss:
                            ...
                            if accelerator.is_main_process:
                                ...
                                console.print(
                                    ...
                                    f"{save_rmse_weight}*{val_rmse:.4f} + "
