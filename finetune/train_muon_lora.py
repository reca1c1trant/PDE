"""
Muon-LoRA Finetuning — Generic script for all 2D datasets.

Uses Newton-Schulz orthogonalization in LoRA forward path.
Same BC + PDE loss as vanilla LoRA, but with orthogonalized low-rank updates.

Supports all PDE types via config (pde_type field).

Usage:
    torchrun --nproc_per_node=4 finetune/train_muon_lora.py \
...
def main():
    ...
    if init_from:
        ...
        if 'trainable_state_dict' in ckpt:
            ...
            # Map PEFT LoRA keys → Muon LoRA keys
            # PEFT: transformer.base_model.model.X.lora_A.default.weight
            # Muon: transformer.X.lora_A
            peft_mapped = {}
            for k, v in src_state.items():
                mapped_k = k
                if 'base_model.model.' in k and '.lora_' in k:
                    mapped_k = k.replace('base_model.model.', '')
                    mapped_k = mapped_k.replace('.default.weight', '')
                peft_mapped[mapped_k] = v

            for k, v in peft_mapped.items():
        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch in train_loader:
                ...
                if global_step % log_interval == 0:
                    ...
                    # Log scale statistics (alpha for muon, lora_s for factored)
                    unwrapped = accelerator.unwrap_model(model)
                    scale_vals = []
                    for name, module in unwrapped.model.transformer.named_modules():
                        if hasattr(module, 'lora_s'):
                            with torch.no_grad():
                                scale_vals.extend(module.lora_s.abs().tolist())
                        elif hasattr(module, 'lora_A') and hasattr(module, 'lora_B'):
                            with torch.no_grad():
                                scale_vals.append((module.lora_B @ module.lora_A).norm().item())
                    if scale_vals:
                        log_dict['train/scale_mean'] = sum(scale_vals) / len(scale_vals)
                        log_dict['train/scale_max'] = max(scale_vals)
                        log_dict['train/scale_min'] = min(scale_vals)
