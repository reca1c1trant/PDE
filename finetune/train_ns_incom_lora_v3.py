"""
LoRA Finetuning for NS Incompressible Inhomogeneous (PDEBench) using PDEModelV3.

Loss: BC loss + No-slip loss + PDE loss (NO RMSE)
- BC loss: RMSE on boundary pixels (supervised anchor)
- No-slip loss: u=0, v=0 at all 4 walls (physics constraint)
- PDE loss: Continuity + Momentum (u,v with force) + Tracer (n-PINN conservative)
...
- Channel 14 = scalar[11] = passive_tracer (particles)
- Channel 15 = scalar[12] = pressure (predicted, not in GT)

Force: time-invariant per-sample field [B, H, W, 2], loaded from HDF5.

Metrics:
- VRMSE = sqrt(MSE / Var(GT)), variance-normalized RMSE
- save_metric = save_vrmse_weight * vrmse + save_pde_weight * pde

Usage:
    torchrun --nproc_per_node=8 finetune/train_ns_incom_lora_v3.py --config configs/finetune_ns_incom_v3.yaml
"""
...
from finetune.pde_loss_verified import NSIncomPDELoss
...
CH_VX = 0       # vector: Vx
CH_VY = 1       # vector: Vy
CH_TRACER = 14  # scalar[11] = passive_tracer (particles)
CH_PRESS = 15   # scalar[12] = pressure


def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for NS Incom Inhom (V3 Model)")
...
def compute_boundary_loss(
    ...
    """
    Compute boundary RMSE on 4 edges (supervised anchor).
    For no-slip walls this provides supervised velocity + tracer signal at boundaries.
    """
...
def compute_noslip_loss(
    output: torch.Tensor,
) -> torch.Tensor:
    """
    No-slip wall loss: enforce u=0, v=0 at all 4 boundaries.
    Physics constraint, no GT needed.
    ...
    return torch.mean(wall_u ** 2) + torch.mean(wall_v ** 2)
...
def compute_pde_loss(
    ...
    force: torch.Tensor,
    pde_loss_fn: NSIncomPDELoss,
) -> tuple:
    """
    Compute NS Incom PDE residual loss (per-equation normalized).

    Extracts u, v, pressure, tracer from model output.
    Prepends t0 frame for time derivatives.
    Includes force field for momentum equations.

    Args:
        output: Model output [B, T_out, H, W, 18] (denormalized)
        input_data: Model input [B, T_in, H, W, 18]
        force: Force field [B, H, W, 2] (time-invariant)
        pde_loss_fn: NSIncomPDELoss instance
    ...
    with torch.autocast(device_type='cuda', enabled=False):
        ...
        t0_d = input_data[:, 0:1, :, :, CH_TRACER].float()

        out_u = output[:, :, :, :, CH_VX].float()
        out_v = output[:, :, :, :, CH_VY].float()
        out_p = output[:, :, :, :, CH_PRESS].float()
        out_d = output[:, :, :, :, CH_TRACER].float()

        # [B, T_out+1, H, W]
        u = torch.cat([t0_u, out_u], dim=1)
        v = torch.cat([t0_v, out_v], dim=1)
        p = torch.cat([t0_p, out_p], dim=1)
        d = torch.cat([t0_d, out_d], dim=1)

        # Force: [B, H, W, 2] → fx [B, H, W], fy [B, H, W]
        fx = force[:, :, :, 0].float()
        fy = force[:, :, :, 1].float()

        total_loss, losses = pde_loss_fn(u, v, p, d, fx, fy)
...
def validate(
    ...
    """Validate and return (bc_loss, noslip_loss, pde_loss, rmse, vrmse)."""
    accelerator.wait_for_everyone()
    model.eval()

    total_bc = torch.zeros(1, device=accelerator.device)
    total_noslip = torch.zeros(1, device=accelerator.device)
    ...
    for batch in val_loader:
        ...
        force = batch['force'].to(device=accelerator.device, dtype=torch.float32)
        ...
        noslip_loss = compute_noslip_loss(output)
        pde_loss, _ = compute_pde_loss(output, input_data, force, pde_loss_fn)

        vrmse, rmse = compute_vrmse(output, target_data, channel_mask)

        total_bc += bc_loss.detach()
        total_noslip += noslip_loss.detach()
    ...
    total_noslip = accelerator.reduce(total_noslip, reduction='sum')
    ...
    avg_noslip = (total_noslip / num_batches).item() if n > 0 else 0
    ...
    return avg_bc, avg_noslip, avg_pde, avg_rmse, avg_vrmse
...
def main():
    ...
    lambda_noslip = config['training'].get('lambda_noslip', 1000.0)
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info(f"NS Incom Inhom LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Loss: BC (supervised) + Noslip (u=v=0) + PDE (normalized cont+mom+tracer)")
        logger.info(f"Lambda BC: {lambda_bc}, Noslip: {lambda_noslip}, PDE: {lambda_pde}")
    ...
    pde_loss_fn = NSIncomPDELoss(
        ...
        dt=physics.get('dt', 0.005),
        nu=physics.get('nu', 0.01),
        skip_bl=physics.get('skip_bl', 2),
    ...
    if args.resume:
        ...
        init_vrmse = metrics.get('vrmse', None)
        init_pde = metrics.get('pde', None)
        if init_vrmse is not None:
            best_val_loss = save_vrmse_weight * init_vrmse + save_pde_weight * (init_pde or 0.0)
        else:
            best_val_loss = ckpt.get('best_val_loss', float('inf'))
        if accelerator.is_main_process:
            logger.info(f"Loaded weights from {args.init_weights}")
            logger.info(f"  checkpoint metrics: vrmse={init_vrmse}, pde={init_pde}")
            logger.info(f"  best_val_loss = {best_val_loss:.6f}")

    if accelerator.is_main_process:
        lora_r = config['model'].get('lora', {}).get('r', 16)
        run_name = f"ns_incom-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["ns_incom", "lora", "v3", "noslip", "BC+PDE", "normalized"],
            }}
        )
        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch in train_loader:
                ...
                with accelerator.accumulate(model):
                    ...
                    # BC loss: supervised boundary RMSE
                    bc_loss = compute_boundary_loss(output, target_data, channel_mask)

                    # No-slip loss: u=v=0 at walls
                    noslip_loss = compute_noslip_loss(output)

                    # PDE loss (normalized cont+mom+tracer)
                    pde_loss, losses = compute_pde_loss(output, input_data, force, pde_loss_fn)

                    # Total loss
                    loss = lambda_bc * bc_loss + lambda_noslip * noslip_loss + lambda_pde * pde_loss
                ...
                noslip_reduced = accelerator.reduce(noslip_loss.detach(), reduction='mean')
                ...
                progress.update(
                    ...
                    description=(
                        ...
                        f"NS={noslip_reduced.item():.4f} PDE={pde_reduced.item():.4f}"
                    )
                )
                ...
                if global_step % eval_interval == 0:
                    val_bc, val_noslip, val_pde, val_rmse, val_vrmse = validate(
                        model, val_loader, accelerator, pde_loss_fn, t_input,
                    )

                    accelerator.log({
                        'val/bc_loss': val_bc,
                        'val/noslip_loss': val_noslip,
                    ...
                    if accelerator.is_main_process:
                        console.print(
                            ...
                            f"pde={val_pde:.6f}, bc={val_bc:.6f}, noslip={val_noslip:.6f}"
                        )
                        ...
                        if save_metric < best_val_loss:
                            ...
                            if accelerator.is_main_process:
                                save_lora_checkpoint(
                                    ...
                                    metrics={
                                        'bc': val_bc, 'noslip': val_noslip,
                                        'pde': val_pde, 'rmse': val_rmse,
                                        'vrmse': val_vrmse,
                                    },
