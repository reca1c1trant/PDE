"""
LoRA Finetuning for Active Matter 2D.
...
Usage:
    torchrun --nproc_per_node=8 finetune/train_active_matter_lora_v3.py --config configs/finetune_active_matter_v3.yaml
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for Active Matter 2D")
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info(f"Active Matter 2D LoRA Finetuning")
        logger.info(f"{'='*60}")
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"active_matter-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["active_matter", "lora", "v3", "BC+PDE", "normalized"],
            }}
        )
