...
from finetune.pde_loss_verified import SwiftHohenberg3DPDELoss

logger = logging.getLogger(__name__)

# Swift-Hohenberg 3D channel index in 18-channel layout
CH_U = 3        # scalar[0] = u (concentration)


def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for 3D Swift-Hohenberg)")
...
def compute_pde_loss(
    ...
    pde_loss_fn: SwiftHohenberg3DPDELoss,
) -> tuple:
    """
    Compute Swift-Hohenberg 3D PDE residual loss.
    Single scalar channel u.
    """
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("2D Burgers (Cole-Hopf) LoRA Finetuning")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Loss: BC (w={bc_width}) + PDE (u_mom + v_mom + irrotational)")
    ...
    pde_loss_fn = SwiftHohenberg3DPDELoss(
        domain_extent=physics.get('domain_extent', 31.41592653589793),
        num_points=physics.get('num_points', 32),
        dt=physics.get('dt', 0.1),
        reactivity=physics.get('reactivity', 0.7),
        critical_number=physics.get('critical_number', 1.0),
        eq_scales=eq_scales,
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"burgers2d_ch-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["burgers_2d_ch", "lora", "v3", "BC+PDE", "normalized"],
            }}
        )
