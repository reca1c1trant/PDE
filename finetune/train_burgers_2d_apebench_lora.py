"""
LoRA Finetuning for APEBench 2D Burgers, periodic).

Loss: BC loss + PDE loss (NO RMSE - that would be cheating!)
- BC loss: RMSE on boundary pixels (supervised anchor)
- PDE loss: u-momentum + v-momentum + irrotational constraint
...
from finetune.pde_loss_verified import APEBenchBurgers2DPDELoss
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for APEBench 2D Burgers)")
...
def compute_pde_loss(
    ...
    pde_loss_fn: APEBenchBurgers2DPDELoss,
) -> tuple:
    """
    Compute APEBench Burgers 2D PDE residual loss.
    No per-sample params needed — coefficients are fixed in pde_loss_fn.
    """
    with torch.autocast(device_type='cuda', enabled=False):
        ...
        total_loss, losses = pde_loss_fn(u, v)
...
def main():
    ...
    pde_loss_fn = APEBenchBurgers2DPDELoss(
        ...
        alpha_2=physics.get('alpha_2', 1.4648e-5),
        delta_conv=physics.get('delta_conv', -2.3438e-3),
        eq_scales=eq_scales,
