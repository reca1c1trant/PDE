"""
LoRA Finetuning for Gray-Scott Reaction-Diffusion using PDEModelV3.
...
Usage:
    torchrun --nproc_per_node=8 finetune/train_gray_scott_lora_v3.py --config configs/finetune_gray_scott_v3.yaml
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for Gray-Scott (V3 Model)")
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("Gray-Scott LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"gray_scott-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["gray_scott", "lora", "v3", "periodic", "BC+PDE", "normalized"],
            }}
        )
