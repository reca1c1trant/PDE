"""
Supervised LoRA Finetuning — Full-Field RMSE Loss (Upper Bound Baseline).

Generic script for all datasets. Loss = RMSE on all valid channels.
No BC loss, no PDE loss — pure supervised training with full ground truth.

Usage:
    torchrun --nproc_per_node=4 finetune/train_supervised_lora.py \
        --config configs/finetune_taylor_green_2d_v3_supervised.yaml
"""
...
def compute_vrmse(
    ...
    """Compute VRMSE and RMSE on valid channels."""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Supervised LoRA Finetuning (RMSE)")
...
@torch.no_grad()
def validate(model, val_loader, accelerator, t_input):
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("Supervised LoRA Finetuning (Full-Field RMSE)")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Loss: RMSE on all valid channels (SUPERVISED)")
        logger.info(f"{'='*60}")
    ...
    if args.resume:
        ...
        best_val_vrmse = ckpt.get('best_val_loss', float('inf'))
    ...
    if accelerator.is_main_process:
        ...
        model_name = config.get('model_name', 'supervised')
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": model_name,
                "tags": ["supervised", "lora", "RMSE", "upper_bound"],
            }},
        )
    ...
    with Progress(
        ...
        train_task = progress.add_task("Supervised LoRA", total=total_steps, completed=global_step)
        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch in train_loader:
                ...
                with accelerator.accumulate(model):
                    ...
                    # Loss = full-field RMSE on all valid channels
                    _, rmse = compute_vrmse(output, target_data, channel_mask)
                    loss = rmse
                ...
                progress.update(train_task, advance=1,
                    description=f"{phase_str} RMSE={loss_reduced.item():.6f}")

                if global_step % log_interval == 0:
                    accelerator.log({
                        'train/rmse': loss_reduced.item(),
                        'train/lr': scheduler.get_last_lr()[0],
                        'train/epoch': epoch + 1,
                    }, step=global_step)

                if global_step % eval_interval == 0:
                    val_rmse, val_vrmse = validate(model, val_loader, accelerator, t_input)
                        ...
                        if val_vrmse < best_val_vrmse:
                            ...
                            if accelerator.is_main_process:
                                save_lora_checkpoint(
                                    ...
                                    best_val_loss=best_val_vrmse,
                                    patience_counter=0,
                                    metrics={'rmse': val_rmse, 'vrmse': val_vrmse},
            ...
            if epoch_steps > 0 and accelerator.is_main_process:
                console.print(f"\n[blue]Epoch {epoch+1}/{max_epochs}:[/blue] "
                              f"avg_rmse={epoch_loss/epoch_steps:.6f}")

    accelerator.end_training()

    if accelerator.is_main_process:
        table = Table(title="Training Complete (Supervised LoRA)", show_header=False)
        table.add_row("Total Steps", str(global_step))
        table.add_row("Best VRMSE", f"{best_val_vrmse:.6f}")
