...
BCs: Periodic in x
Per-sample a: from dataset nu field (stores advection velocity)
...
Usage:
    torchrun --nproc_per_node=4 finetune/train_advection_1d_lora_v3.py \
...
from finetune.pde_loss_verified import Advection1DPDELoss

logger = logging.getLogger(__name__)

# 1D Advection channel index: u -> scalar[11] = channel 14 (3+11)
CH_U = 14


def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for 1D Advection (V3 Model)")
...
def compute_pde_loss(
    ...
    pde_loss_fn: Advection1DPDELoss,
    a: torch.Tensor,
) -> tuple:
    """
    ...
    Args:
        ...
        pde_loss_fn: Advection1DPDELoss instance
    ...
    with torch.autocast(device_type='cuda', enabled=False):
        ...
        total_loss, losses = pde_loss_fn(u, a=a.float())
...
def validate(
    ...
    for batch in val_loader:
        ...
        a = batch['nu'].to(device=accelerator.device)  # nu stores advection velocity
        ...
        pde_loss, _ = compute_pde_loss(output, input_data, pde_loss_fn, a)
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("1D Advection LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Loss: BC (w={bc_width}) + PDE (u_t + a*u_x, 4th-order central)")
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"advection1d-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["advection_1d", "lora", "v3", "BC+PDE", "normalized"],
            }}
        )
        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch in train_loader:
                ...
                with accelerator.accumulate(model):
                    ...
                    bc_loss = compute_boundary_loss(output, target_data, channel_mask, bc_width=bc_width)
                    pde_loss, losses = compute_pde_loss(output, input_data, pde_loss_fn, a)
