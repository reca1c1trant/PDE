"""
LoRA Finetuning for APEBench 1D Burgers using PDEModelV3.

Loss: BC loss + PDE loss (NO RMSE)
- BC loss: RMSE on boundary pixels (supervised anchor for periodic domain)
- PDE loss: u_t + u*u_x - nu*u_xx = 0, per-sample nu

Channel mapping (18-channel):
- Channel 0 = Vx (vector[0], velocity u)
- vector_dim=1
...
Usage:
    torchrun --nproc_per_node=4 finetune/train_burgers_1d_lora_v3.py \
...
from finetune.pde_loss_verified import APEBenchBurgers1DPDELoss

logger = logging.getLogger(__name__)

# 1D Burgers channel index: u -> vector[0]
CH_VX = 0


def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for APEBench 1D Burgers (V3 Model)")
...
def compute_pde_loss(
    ...
    pde_loss_fn: APEBenchBurgers1DPDELoss,
) -> tuple:
    """
    ...
    Args:
        ...
        pde_loss_fn: APEBenchBurgers1DPDELoss instance
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("1D Burgers LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Loss: BC (w={bc_width}) + PDE (u_t + u*u_x - nu*u_xx)")
    ...
    pde_loss_fn = APEBenchBurgers1DPDELoss(
        ...
        alpha_2=physics.get('alpha_2', 2.9297e-5),
        delta_conv=physics.get('delta_conv', -4.6875e-3),
        eq_scales=eq_scales,
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"burgers1d-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["burgers_1d", "lora", "v3", "BC+PDE", "normalized"],
            }}
        )
