"""
LoRA Finetuning for KP-II (Kadomtsev-Petviashvili) using PDEModelV3.

Loss: BC loss + PDE loss (NO RMSE)
- BC loss: RMSE on boundary pixels (supervised anchor for periodic domain)
- PDE loss: KP-II equation residual (expanded form), per-equation normalized

Channel mapping (18-channel):
- Channel 3 = scalar[0] = u (wave amplitude)
...
Metrics:
- VRMSE: variance-normalized RMSE (for monitoring)
- save_metric = save_vrmse_weight * val_vrmse + save_pde_weight * val_pde

Usage:
    torchrun --nproc_per_node=4 finetune/train_kp2_2d_lora_v3.py --config configs/finetune_kp2_2d_v3.yaml
"""
...
from finetune.pde_loss_verified import KP2DPDELoss

logger = logging.getLogger(__name__)


# KP-II channel index in 18-channel layout
# scalar_indices=[0] means scalar[0] -> ch3 in 18-channel
CH_U = 3  # scalar[0] = wave amplitude u


def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for KP-II (V3 Model)")
...
def compute_pde_loss(
    ...
    Args:
        ...
        pde_loss_fn: KP2DPDELoss instance
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("KP-II LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Loss: BC (boundary w={bc_width}) + PDE (KP-II expanded)")
    ...
    pde_loss_fn = KP2DPDELoss(
        ...
        dx=physics.get('dx', 20.0 / 256),
        dy=physics.get('dy', 20.0 / 256),
        dt=physics.get('dt', 0.01),
        skip_boundary=physics.get('skip_boundary', 20),
        eq_scales=eq_scales,
    ...
    if accelerator.is_main_process:
        ...
        logger.info(f"PDE skip_boundary: {pde_loss_fn.skip_boundary}")
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"kp2_2d-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["kp2", "lora", "v3", "periodic", "BC+PDE", "normalized"],
            }}
        )
