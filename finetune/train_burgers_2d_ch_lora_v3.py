"""
LoRA Finetuning for 2D Burgers (Cole-Hopf, periodic).
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for 2D Burgers (Cole-Hopf)")
...
def compute_pde_loss(
    ...
    """
    Compute Burgers 2D PDE residual loss (per-equation normalized).

    Extracts u, v from denormalized model output,
    ...
    with torch.autocast(device_type='cuda', enabled=False):
        ...
        total_loss, losses = pde_loss_fn(u, v, nu=nu.float(), t_offset=t_offset)
