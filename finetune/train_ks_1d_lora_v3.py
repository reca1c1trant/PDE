"""
LoRA Finetuning for APEBench 1D Kuramoto-Sivashinsky using PDEModelV3.

Loss: BC loss + PDE loss (NO RMSE)
- BC loss: RMSE on boundary pixels (supervised anchor for periodic domain)
- PDE loss: u_t = alpha_2*u_xx + alpha_4*u_xxxx + beta_2*1/2*(u_x)^2

Channel mapping (18-channel):
- scalar_indices = [0] -> Channel 3 (first scalar channel)
- vector_dim=0 (pure scalar PDE, no vector channels active)

BCs: Periodic in x
No per-sample parameter variation.
...
Usage:
    torchrun --nproc_per_node=4 finetune/train_ks_1d_lora_v3.py \
        --config configs/finetune_ks_1d_v3.yaml
"""
...
from finetune.pde_loss_verified import KS1DPDELoss

logger = logging.getLogger(__name__)

# KS is a scalar PDE stored in scalar_indices=[0] -> channel 3 in 18-ch layout
CH_SCALAR0 = 3


def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for 1D KS (V3 Model)")
...
def compute_boundary_loss(
    ...
    """
    Compute boundary RMSE on border strips of width `bc_width` for 1D data.

    1D data layout: [B, T, X, 18]
    For periodic domains: compare left/right boundary strips.
    """
    ...
    w = bc_width
    left_pred = pred[:, :, :w, :][:, :, :, valid_ch]
    left_target = target[:, :, :w, :][:, :, :, valid_ch]
    right_pred = pred[:, :, -w:, :][:, :, :, valid_ch]
    right_target = target[:, :, -w:, :][:, :, :, valid_ch]

    bc_pred = torch.cat([left_pred.reshape(-1), right_pred.reshape(-1)])
    bc_target = torch.cat([left_target.reshape(-1), right_target.reshape(-1)])
...
def compute_pde_loss(
    ...
    Args:
        output: Model output [B, T_out, X, 18] (denormalized)
        input_data: Model input [B, T_in, X, 18]
        pde_loss_fn: KS1DPDELoss instance

    Returns:
        total_loss, losses_dict
    """
    with torch.autocast(device_type='cuda', enabled=False):
        # Prepend t0 frame; KS scalar is at channel CH_SCALAR0
        t0_u = input_data[:, 0:1, :, CH_SCALAR0].float()
        out_u = output[:, :, :, CH_SCALAR0].float()

        # [B, T_out+1, X] for 2nd-order central time difference
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("1D Kuramoto-Sivashinsky LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Loss: BC (w={bc_width}) + PDE (KS: u_t = a2*u_xx + a4*u_xxxx + b2*1/2*(u_x)^2)")
    ...
    train_loader, val_loader, train_sampler, val_sampler = create_finetune_dataloaders(
        ...
        vector_dim=config['dataset'].get('vector_dim', 1),
    ...
    pde_loss_fn = KS1DPDELoss(
        nx=physics.get('nx', 160),
        Lx=physics.get('Lx', 1.0),
        dt=physics.get('dt', 1.0),
        alpha_2=physics.get('alpha_2', -2.34375e-5),
        alpha_4=physics.get('alpha_4', -2.8610229492e-9),
        beta_2=physics.get('beta_2', -2.34375e-4),
        eq_scales=eq_scales,
        eq_weights=eq_weights,
    ).to(accelerator.device)

    if accelerator.is_main_process:
        logger.info(f"PDE alpha_2: {pde_loss_fn.alpha_2}")
        logger.info(f"PDE alpha_4: {pde_loss_fn.alpha_4}")
        logger.info(f"PDE beta_2: {pde_loss_fn.beta_2}")
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"ks1d-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["ks_1d", "lora", "v3", "BC+PDE", "normalized"],
            }}
        )
