"""
LoRA Finetuning for Taylor-Green Vortex 2D (Incompressible Navier-Stokes).

Loss: BC loss + PDE loss (NO RMSE - that would be cheating!)
- BC loss: RMSE on boundary pixels (supervised anchor)
- PDE loss: Continuity (div u=0) + x-momentum + y-momentum
...
Per-sample nu: from dataset nu field
...
Usage:
    torchrun --nproc_per_node=4 finetune/train_taylor_green_2d_lora_v3.py --config configs/finetune_taylor_green_2d_v3.yaml
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for Taylor-Green 2D")
    parser.add_argument('--config', type=str, required=True, help='Path to config file')
    parser.add_argument('--resume', type=str, default=None, help='Path to checkpoint to resume')
    parser.add_argument('--reset_patience', action='store_true', help='Reset early stopping')
    parser.add_argument('--init_weights', type=str, default=None, help='Path to init LoRA weights')
...
def compute_pde_loss(
    ...
    pde_loss_fn: TaylorGreen2DPDELoss,
    nu: torch.Tensor,
    t_offset: Optional[torch.Tensor] = None,
) -> tuple:
    """
    Compute Taylor-Green PDE residual loss (per-equation normalized).

    Extracts u, v, p from denormalized model output,
    prepends t0 frame for time derivatives.
    NO spatial transpose (unlike RB).
    """
    with torch.autocast(device_type='cuda', enabled=False):
        ...
        # NO spatial transpose needed for Taylor-Green (both dims periodic, square domain)

        total_loss, losses = pde_loss_fn(u, v, p, nu=nu.float(), t_offset=t_offset)
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info(f"Taylor-Green 2D LoRA Finetuning")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Loss: BC (w={bc_width}) + PDE (continuity + x-mom + y-mom)")
    ...
    if args.init_weights:
        ...
        if accelerator.is_main_process:
            logger.info(f"Loaded init weights from {args.init_weights}")

    if accelerator.is_main_process:
        lora_r = config['model'].get('lora', {}).get('r', 16)
        run_name = f"taylor_green_2d-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["taylor_green_2d", "lora", "v3", "BC+PDE", "normalized"],
            }}
        )
