"""
LoRA Finetuning for 2D Wave Equation using PDEModelV3.

Loss: BC loss + PDE loss (NO RMSE)
- BC loss: RMSE on boundary pixels (supervised anchor for periodic domain)
- PDE loss: Wave equation residual (u_eq + w_eq), per-equation normalized
...
- Channel 3 = scalar[0] = u (displacement)
- Channel 4 = scalar[1] = w (velocity = u_t)
- No velocity fields (vector_dim=0)

Parameters:
- c (wave speed) varies per sample, stored in 'nu' field

Metrics:
- VRMSE: per-channel normalized RMSE (monitoring only)
- save_metric = save_vrmse_weight * val_vrmse + save_pde_weight * val_pde

Usage:
    torchrun --nproc_per_node=4 finetune/train_wave_2d_lora_v3.py --config configs/finetune_wave_2d_v3.yaml
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for 2D Wave Equation (V3 Model)")
...
def compute_pde_loss(
    ...
    Args:
        ...
        pde_loss_fn: Wave2DPDELoss instance
        c: Per-sample wave speed [B]
...
def compute_vrmse(
    ...
    for c_idx in range(n_ch):
        pred_c = output[..., valid_ch[c_idx]]
        gt_c = target[..., valid_ch[c_idx]]
...
def validate(
    ...
    for batch in val_loader:
        ...
        c_vals = batch['nu'].to(device=accelerator.device, dtype=torch.float32)
        ...
        pde_loss, _ = compute_pde_loss(output, input_data, pde_loss_fn, c_vals)
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("2D Wave Equation LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Loss: BC (boundary w={bc_width}) + PDE (u_eq + w_eq)")
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"wave_2d-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["wave_2d", "lora", "v3", "periodic", "BC+PDE", "normalized"],
            }}
        )
        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch in train_loader:
                ...
                with accelerator.accumulate(model):
                    ...
                    # PDE loss (u_eq + w_eq, per-equation normalized)
                    pde_loss, losses = compute_pde_loss(output, input_data, pde_loss_fn, c_vals)
