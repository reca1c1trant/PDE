...
Usage:
    torchrun --nproc_per_node=4 finetune/train_aniso_diff_2d_lora_v3.py \\
        --config configs/finetune_aniso_diff_2d_v3.yaml
"""
...
from finetune.pde_loss_verified import AnisoDiff2DPDELoss

logger = logging.getLogger(__name__)


# Anisotropic Diffusion channel index in 18-channel layout
CH_U = 3  # scalar[0] = scalar field u


def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for Anisotropic Diffusion 2D (V3 Model)")
...
def compute_pde_loss(
    ...
    pde_loss_fn: AnisoDiff2DPDELoss,
) -> tuple:
    """
    ...
    Args:
        ...
        pde_loss_fn: AnisoDiff2DPDELoss instance
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("Anisotropic Diffusion 2D LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Loss: BC (boundary w={bc_width}) + PDE (aniso_diff)")
    ...
    pde_loss_fn = AnisoDiff2DPDELoss(
        ...
        ny=physics.get('ny', 160),
        dx=physics.get('dx', 1.0 / 160),
        dy=physics.get('dy', 1.0 / 160),
        dt=physics.get('dt', 0.1),
        A_xx=physics.get('A_xx', 0.001),
        A_xy=physics.get('A_xy', 0.0005),
        A_yx=physics.get('A_yx', 0.0005),
        A_yy=physics.get('A_yy', 0.002),
        eq_scales=eq_scales,
    ...
    if accelerator.is_main_process:
        ...
        logger.info(f"PDE params: A_xx={pde_loss_fn.A_xx}, A_xy={pde_loss_fn.A_xy}, "
                     f"A_yx={pde_loss_fn.A_yx}, A_yy={pde_loss_fn.A_yy}")
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"aniso_diff_2d-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["aniso_diff_2d", "lora", "v3", "periodic", "BC+PDE", "normalized"],
            }}
        )
        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch in train_loader:
                ...
                with accelerator.accumulate(model):
                    ...
                    # PDE loss (anisotropic diffusion, per-equation normalized)
