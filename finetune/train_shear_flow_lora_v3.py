"""
LoRA Finetuning for Shear Flow (Incompressible NS + Tracer) using PDEModelV3.
...
Usage:
    torchrun --nproc_per_node=8 finetune/train_shear_flow_lora_v3.py --config configs/finetune_shear_flow_v3.yaml
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for Shear Flow (V3 Model)")
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("Shear Flow LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"shear_flow-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["shear_flow", "lora", "v3", "periodic", "BC+PDE", "normalized"],
            }}
        )
