"""
LoRA Finetuning for Turbulent Radiative Layer 2D (Compressible Euler).

Loss: BC loss + PDE loss (NO RMSE - that would be cheating!)
- BC loss: RMSE on boundary pixels (supervised anchor)
- PDE loss: Mass conservation + x-Momentum + y-Momentum
  Per-equation normalized by GT RMS scale -> all equations ~O(1)
  (Energy equation NOT used: t_cool << dt makes source term unresolvable)

Channel mapping (18-channel):
- Channel 0 = Vx, Channel 1 = Vy (vector_dim=2)
- Channel 7 = scalar[4] = density
- Channel 15 = scalar[12] = pressure

BCs: x periodic, y open (Neumann)
Grid: 128x384, x in [-0.5, 0.5], y in [-1, 2]
Per-sample tcool stored in nu field (NOT used in PDE loss).
...
Usage:
    torchrun --nproc_per_node=8 finetune/train_turbulent_radiative_2d_lora_v3.py --config configs/finetune_turbulent_radiative_2d_v3.yaml
"""
...
# TRL 2D channel indices in 18-channel layout
CH_VX = 0       # vector: Vx
CH_VY = 1       # vector: Vy
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for Turbulent Radiative Layer 2D")
...
def compute_patch_boundary_loss(
    output: torch.Tensor,
    channel_mask: torch.Tensor,
    patch_size: int = 16,
) -> torch.Tensor:
    """
    Penalize non-smoothness at patch boundaries via 2nd derivative (curvature).

    Computes |f[P-2] - 2*f[P-1] + f[P]|^2 at each patch boundary.
    Self-supervised: only uses model output, no GT needed.
    """
    ...
    # Horizontal boundaries (along W dimension)
    h_curv = x[:, :, :, P - 2:-2:P, :] - 2 * x[:, :, :, P - 1:-1:P, :] + x[:, :, :, P::P, :]
    # Vertical boundaries (along H dimension)
    v_curv = x[:, :, P - 2:-2:P, :, :] - 2 * x[:, :, P - 1:-1:P, :, :] + x[:, :, P::P, :, :]

    return torch.mean(h_curv ** 2) + torch.mean(v_curv ** 2)
...
def compute_pde_loss(
    ...
    """
    Compute TRL PDE residual loss (compressible Euler: mass + momentum).

    Per-equation normalization handled inside TurbulentRadiativePDELoss via eq_scales/eq_weights.
    """
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"PDE: nx={pde_loss_fn.nx}, ny={pde_loss_fn.ny}, "
                     f"dx={pde_loss_fn.dx:.6f}, dy={pde_loss_fn.dy:.6f}, dt={pde_loss_fn.dt}")
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"turbulent_radiative_2d-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["turbulent_radiative_2d", "lora", "v3", "BC+PDE", "normalized"],
            }}
        )
            ...
            for batch in train_loader:
                ...
                with accelerator.accumulate(model):
                    ...
                    if lambda_boundary > 0:
                        boundary_loss = compute_patch_boundary_loss(output, channel_mask, patch_size)
