"""
SUT-FNO Training Script.

Train Spectral U-Net Transformer + FNO model on Burgers equation with:
- PDE residual loss (physics-informed)
- RMSE loss (supervised)

Usage:
    # Single GPU
    python train_sut_fno.py --config configs/sut_fno_burgers.yaml

    # Multi-GPU
    torchrun --nproc_per_node=8 train_sut_fno.py --config configs/sut_fno_burgers.yaml
"""
...
from model_sut_fno import create_sut_fno_model
...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/sut_fno_burgers.yaml')
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info(f"SUT-FNO Training - Burgers Equation")
        logger.info(f"{'='*60}")
    ...
    # Create model
    model = create_sut_fno_model(config)

    # Log parameter counts
    if accelerator.is_main_process:
        params = model.count_parameters()
        print(f"\n{'='*60}")
        print(f"SUT-FNO Model Parameters")
        print(f"{'='*60}")
    ...
    # Init WandB
    if accelerator.is_main_process:
        run_name = f"sut-fno-pde{lambda_pde}-rmse{lambda_rmse}"
    ...
    with Progress(
        ...
        for epoch in range(max_epochs):
            ...
            for batch in train_loader:
                ...
                desc = f"{phase_str} PDE={pde_loss_reduced.item():.4f} RMSE={rmse_loss_reduced.item():.4f}"
                ...
                if global_step % log_interval == 0:
                    log_dict = {
                        ...
                        'train/total_loss': (lambda_pde * pde_loss_reduced + lambda_rmse * rmse_loss_reduced).item(),
                ...
                if global_step % eval_interval == 0:
                    accelerator.wait_for_everyone()
                    val_pde, val_rmse = validate(model, val_loader, config, accelerator)

                    accelerator.log({
                        'val/pde_loss': val_pde,
                        'val/rmse_loss': val_rmse,
                        'val/total_loss': lambda_pde * val_pde + lambda_rmse * val_rmse,
                    }, step=global_step)

                    if accelerator.is_main_process:
                        console.print(
                            f"\n[green]Step {global_step}/{total_steps} {phase_str}:[/green] "
                            f"val_pde={val_pde:.6f}, val_rmse={val_rmse:.6f}"
                        )
                        ...
                        if val_loss < best_val_loss:
                            ...
                            if accelerator.is_main_process:
                                ...
                                save_checkpoint(
                                    ...
                                    scheduler=scheduler,
                                    global_step=global_step,
                                    metrics={'pde_loss': val_pde, 'rmse_loss': val_rmse},
            ...
            if epoch_steps > 0:
                ...
                if accelerator.is_main_process:
                    console.print(
                        ...
                        f"avg_pde={avg_pde:.6f}, avg_rmse={avg_rmse:.6f}"
                    )

    accelerator.end_training()

    if accelerator.is_main_process:
        table = Table(title="SUT-FNO Training Complete", show_header=False, border_style="green")
