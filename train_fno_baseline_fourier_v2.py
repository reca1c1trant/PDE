"""
FNO Baseline Training Script V2.
...
Loss = lambda_rmse * RMSE + lambda_pde(t) * PDE + lambda_bc * Boundary

Usage:
    torchrun --nproc_per_node=8 train_fno_baseline_fourier_v2.py --config configs/fno_baseline_burgers_v2.yaml
"""
...
from pde_loss_fourier_v2 import burgers_pde_loss_fourier_v2, boundary_loss
...
def get_progressive_pde_weight(
    ...
    Allows model to first learn basic prediction (RMSE),
    then gradually enforce physics (PDE).
    """
...
def compute_pde_loss(output, input_data, batch, config, accelerator):
    """Compute both normalized and raw PDE loss using Fourier derivatives.

    Returns both for:
    - normalized: used for training (stable gradients)
    - raw: used for comparison with previous experiments
    """
    ...
    # Normalized PDE loss (for training)
    pde_loss_norm, loss_u_norm, loss_v_norm = burgers_pde_loss_fourier_v2(
    ...
    # Raw PDE loss (for comparison)
    pde_loss_raw, _, _ = burgers_pde_loss_fourier_v2(
        pred=pred_with_t0.float(),
        nu=nu,
        dt=dt,
        dx=dx,
    ...
    return pde_loss_norm, pde_loss_raw, loss_u_norm, loss_v_norm
...
def validate(model, val_loader, config, accelerator, lambda_pde, lambda_bc):
    ...
    Returns:
        val_pde_norm: normalized PDE loss (for training comparison)
        val_pde_raw: raw PDE loss (for comparison with previous experiments)
        val_rmse: RMSE loss
        val_bc: boundary loss
    """
    model.eval()

    total_pde_loss_norm = torch.zeros(1, device=accelerator.device)
    total_pde_loss_raw = torch.zeros(1, device=accelerator.device)
    ...
    for batch in val_loader:
        ...
        pde_loss_norm, pde_loss_raw, _, _ = compute_pde_loss(output, input_data, batch, config, accelerator)
        rmse_loss = compute_rmse_loss(output, target)
        bc_loss = compute_boundary_loss(output, target)

        total_pde_loss_norm += pde_loss_norm.detach()
        total_pde_loss_raw += pde_loss_raw.detach()
    ...
    total_pde_loss_norm = accelerator.reduce(total_pde_loss_norm, reduction='sum')
    total_pde_loss_raw = accelerator.reduce(total_pde_loss_raw, reduction='sum')
    ...
    n = num_batches.item()
    return (
        (total_pde_loss_norm / n).item() if n > 0 else 0,
        (total_pde_loss_raw / n).item() if n > 0 else 0,
...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/fno_baseline_burgers_v2.yaml')
    ...
    warmup_steps = config['training'].get('warmup_steps', 200)
    pde_warmup_steps = config['training'].get('pde_warmup_steps', 1000)  # V2: PDE warmup
    lambda_pde_max = config['training'].get('lambda_pde', 1.0)
    lambda_rmse = config['training'].get('lambda_rmse', 1.0)
    lambda_bc = config['training'].get('lambda_bc', 1.0)  # V2: boundary loss
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info(f"FNO Baseline V2 - PINO-aligned Architecture")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Model: Residual + PosEmbed({use_pos_embed}) + ChannelMLP({use_channel_mlp})")
        logger.info(f"PDE Loss: Fourier (normalized)")
    ...
    if accelerator.is_main_process:
        ...
        print(f"\n{'='*60}")
        print(f"FNO Baseline V2 Model Parameters")
        print(f"{'='*60}")
    ...
    # Init WandB
    if accelerator.is_main_process:
        run_name = f"fno-v2-residual-posembed-mlp"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["fno-v2", "pino-aligned", "residual", "pos-embed", "channel-mlp"],
            }}
        )
    ...
    with Progress(
        ...
        train_task = progress.add_task("FNO V2", total=total_steps)
        ...
        for epoch in range(max_epochs):
            ...
            for batch in train_loader:
                ...
                with accelerator.accumulate(model):
                    ...
                    # Losses (normalized for training, raw for comparison)
                    pde_loss_norm, pde_loss_raw, loss_u, loss_v = compute_pde_loss(
                        output, input_data, batch, config, accelerator
                    )
                    rmse_loss = compute_rmse_loss(output, target)
                    bc_loss = compute_boundary_loss(output, target)

                    # Total loss with progressive PDE weight (use normalized)
                    train_loss = (
                        lambda_rmse * rmse_loss +
                        lambda_pde * pde_loss_norm +
                        lambda_bc * bc_loss
                    )
                ...
                # Reduce for logging
                pde_loss_norm_reduced = accelerator.reduce(pde_loss_norm.detach(), reduction='mean')
                pde_loss_raw_reduced = accelerator.reduce(pde_loss_raw.detach(), reduction='mean')
                ...
                epoch_pde_loss += pde_loss_norm_reduced.item()
                epoch_pde_loss_raw += pde_loss_raw_reduced.item()
                epoch_rmse_loss += rmse_loss_reduced.item()
                epoch_bc_loss += bc_loss_reduced.item()

                # Update progress (show raw PDE for comparison)
                phase_str = "[warmup]" if in_warmup else f"[E{epoch+1}]"
                desc = f"{phase_str} PDE_raw={pde_loss_raw_reduced.item():.1f} PDE_norm={pde_loss_norm_reduced.item():.4f}(w={lambda_pde:.2f}) RMSE={rmse_loss_reduced.item():.4f}"
                progress.update(train_task, advance=1, description=desc)

                # Log to wandb
                if global_step % log_interval == 0:
                    log_dict = {
                        'train/pde_loss': pde_loss_norm_reduced.item(),        # normalized (for training)
                        'train/pde_loss_raw': pde_loss_raw_reduced.item(),     # raw (for comparison)
                        ...
                        'train/total_loss': (
                            ...
                            lambda_pde * pde_loss_norm_reduced +
                ...
                if global_step % eval_interval == 0:
                    accelerator.wait_for_everyone()
                    val_pde_norm, val_pde_raw, val_rmse, val_bc = validate(
                        model, val_loader, config, accelerator, lambda_pde, lambda_bc
                    )

                    accelerator.log({
                        'val/pde_loss': val_pde_norm,          # normalized (for training)
                        'val/pde_loss_raw': val_pde_raw,       # raw (for comparison)
                        'val/rmse_loss': val_rmse,
                        'val/bc_loss': val_bc,
                        'val/total_loss': lambda_rmse * val_rmse + lambda_pde * val_pde_norm + lambda_bc * val_bc,
                    }, step=global_step)

                    if accelerator.is_main_process:
                        console.print(
                            f"\n[green]Step {global_step}/{total_steps} {phase_str}:[/green] "
                            f"val_pde_raw={val_pde_raw:.1f}, val_pde_norm={val_pde_norm:.6f}, val_rmse={val_rmse:.6f}, val_bc={val_bc:.6f}"
                        )

                    val_loss = lambda_rmse * val_rmse + lambda_pde * val_pde_norm + lambda_bc * val_bc
                        ...
                        if val_loss < best_val_loss:
                            ...
                            if accelerator.is_main_process:
                                ...
                                save_checkpoint(
                                    ...
                                    global_step=global_step,
                                    metrics={
                                        'pde_loss': val_pde_norm,
                                        'pde_loss_raw': val_pde_raw,
                                        'rmse_loss': val_rmse,
                                        'bc_loss': val_bc,
                                    },
                                ...
                                console.print(f"[yellow]Saved best model[/yellow] (val_loss: {val_loss:.6f}, pde_raw: {val_pde_raw:.1f})")
                        else:
                            patience_counter += 1
                            if accelerator.is_main_process:
            ...
            if epoch_steps > 0:
                ...
                if accelerator.is_main_process:
                    console.print(
                        ...
                        f"avg_pde_raw={avg_pde_raw:.1f}, avg_pde_norm={avg_pde:.6f}, avg_rmse={avg_rmse:.6f}, avg_bc={avg_bc:.6f}"
                    )

    accelerator.end_training()

    if accelerator.is_main_process:
        table = Table(title="FNO V2 Training Complete", show_header=False, border_style="green")
