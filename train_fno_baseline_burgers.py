"""
Baseline Training Script - Burgers Equation.

Train UNet or MLP model on 2D Burgers equation.
Supports:
- V1: Non-conservative form (pde_loss.py)
- V2: Conservative flux form (pde_loss_burgers_v2.py, n-PINN style)

Loss: lambda_pde * PDE + lambda_bc * BC (no RMSE)

Usage:
    # Single GPU
    python train_fno_baseline_burgers.py --config configs/unet_baseline_burgers_v2.yaml

    # Multi-GPU
    torchrun --nproc_per_node=8 train_fno_baseline_burgers.py --config configs/unet_baseline_burgers_v2.yaml
"""
...
def compute_pde_loss(output, input_data, batch, config, accelerator):
    ...
    with torch.autocast(device_type='cuda', enabled=False):
        # Use first two channels (u, v) for PDE loss
        t0_frame = input_data[:, 0:1, ..., :2].float()  # [B, 1, H, W, 2]
        output_uv = output[..., :2].float()  # [B, T-1, H, W, 2]
        pred_with_t0 = torch.cat([t0_frame, output_uv], dim=1)
        pred_uv = pred_with_t0  # Already float32

        # Boundaries have 2 channels (u, v)
...
def compute_rmse_loss(output, target):
    """Compute RMSE loss between output and ground truth (for monitoring only)."""
    with torch.autocast(device_type='cuda', enabled=False):
        output_f32 = output[..., :2].float()
        target_f32 = target[..., :2].float()
...
def validate(model, val_loader, config, accelerator):
    ...
    for batch in val_loader:
        # Use first two channels (u, v)
        data = batch['data'][..., :2].to(device=accelerator.device, dtype=torch.float32)
        input_data = data[:, :-1]
        target = data[:, 1:]
...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/unet_baseline_burgers.yaml')
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info(f"Baseline Training - Burgers Equation (n-PINN)")
        logger.info(f"{'='*60}")
    ...
    # Init WandB
    if accelerator.is_main_process:
        run_name = f"{model_type}-baseline-burgers"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": [f"{model_type}-baseline", "burgers", "n-pinn"],
            }}
        )
    ...
    with Progress(
        ...
        train_task = progress.add_task("Burgers Baseline", total=total_steps)
        ...
        for epoch in range(max_epochs):
            ...
            for batch in train_loader:
                ...
                with accelerator.accumulate(model):
                    ...
                    # RMSE loss (for monitoring only)
                    rmse_loss = compute_rmse_loss(output, target)

                    # Boundary loss
                    bc_loss = compute_boundary_loss(output, target)

                    # Total loss (no RMSE in training)
    ...
    if accelerator.is_main_process:
        table = Table(title=f"{model_type.upper()} Baseline Training Complete (Burgers)", show_header=False, border_style="green")
        table.add_row("Model Type", model_type.upper())
        table.add_row("PDE Loss", "n-PINN conservative flux")
