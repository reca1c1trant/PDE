        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch in train_loader:
                ...
                if should_validate:
                    ...
                    # Weighted val metric for best-model saving
                    if val_weights and per_dataset:
                        w_sum = sum(per_dataset[ds] * val_weights.get(ds, 1.0) for ds in per_dataset)
                        w_total = sum(val_weights.get(ds, 1.0) for ds in per_dataset)
                        val_metric = w_sum / w_total if w_total > 0 else val_rmse
                    else:
                        val_metric = val_rmse
                    ...
                    if accelerator.is_main_process:
                        ...
                        metric_str = f"weighted={val_metric:.6f}" if val_weights else ""
                        console.print(f"\n[green]Step {global_step}/{total_steps} {phase_str}:[/green] val_rmse={val_rmse:.6f} {metric_str}")
                        if ds_str:
