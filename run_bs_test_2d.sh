#!/bin/bash
#SBATCH --job-name=bs_test_2d
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=10
#SBATCH --time=00:10:00
#SBATCH --output=logs_1d3d/bs_test_2d_%j.out
#SBATCH --error=logs_1d3d/bs_test_2d_%j.err
...
# 2D Burgers: 160x160, 18 channels, t_input=8
config = {
    'model': {
...
model = PDELoRAModelV3(
    config=config,
    pretrained_path='./checkpoints_v4_2donly_postsmooth/best_tf.pt',
    freeze_encoder=False, freeze_decoder=False,
).cuda().float()

print('Model loaded. Testing batch sizes for 2D (160x160)...')
for bs in [2, 4, 8, 12, 16, 20]:
    torch.cuda.empty_cache()
    try:
        x = torch.randn(bs, 8, 160, 160, 18).cuda().float()
        with torch.no_grad():
        ...
        print(f'  bs={bs}: OK, mem={mem:.1f}GB')
...
# 1D Burgers: 160, 18 channels
print()
print('Testing batch sizes for 1D (160)...')
config['model']['enable_1d'] = True
model = PDELoRAModelV3(
    config=config,
    pretrained_path='./checkpoints_v4_s_3d_with_best1-2d/best_tf.pt',
    freeze_encoder=False, freeze_decoder=False,
).cuda().float()
...
for bs in [4, 8, 16, 32, 64, 128, 256]:
    torch.cuda.empty_cache()
    try:
        x = torch.randn(bs, 8, 160, 18).cuda().float()
        with torch.no_grad():
            out, m, s = model(x, return_normalized=True)
        loss = out.sum()
        loss.backward()
