"""
LoRA V3 Model Visualization for Taylor-Green 2D (Incompressible Navier-Stokes).
...
Usage:
    ...
    torchrun --nproc_per_node=4 tools/visualize_taylor_green_2d_lora.py \
        ...
        --checkpoint checkpoints_taylor_green_2d_lora_v3/best_lora.pt --scan_all

    # Single-GPU visualization (plot only)
    python tools/visualize_taylor_green_2d_lora.py \
        --config configs/finetune_taylor_green_2d_v3.yaml \
        --checkpoint checkpoints_taylor_green_2d_lora_v3/best_lora.pt --output_dir ./taylor_green_2d_vis
"""
...
def compute_pde_loss_from_output(
    ...
    """Compute Taylor-Green PDE residual loss."""
...
def scan_all_distributed(accelerator, model, val_loader, config, t_input):
    ...
    return {'pde': 0, 'rmse_vx': 0, 'rmse_vy': 0,
            'rmse_press': 0, 'rmse': 0, 'vrmse_vx': 0, 'vrmse_vy': 0,
            'vrmse_press': 0, 'nrmse_vx': 0, 'nrmse_vy': 0,
            'nrmse_press': 0,
...
def run_visualization(model, dataset, config, device, t_input, num_samples, seed, output_dir):
    ...
    for i, sample_idx in enumerate(sample_indices):
        ...
        print(f"  Sample {sample_idx}: t_start={start_t}, "
              ...
              f"nRMSE_p={nrmse_press:.6f}, PDE={pde_loss:.6f}")

    print("Plotting velocity visualization...")
    plot_velocity(results, str(output_dir / "visualization_taylor_green_2d_velocity.png"))

    print("Plotting scalar visualization...")
    plot_scalar(results, str(output_dir / "visualization_taylor_green_2d_scalar.png"))

    print(f"\n{'='*60}")
    print("Visualization Complete (Taylor-Green 2D LoRA)")
    print(f"{'='*60}")
...
def plot_scalar(results: list, save_path: str):
    ...
    for row, res in enumerate(results):
        ...
        im0 = axes[row, 0].imshow(gt_p, cmap='RdBu_r',
                                    vmin=vmin_p, vmax=vmax_p)
        axes[row, 0].set_title('GT (pressure)', fontsize=11)
        axes[row, 0].set_ylabel(f'Sample {sample_idx}\nt0={start_t}', fontsize=10)
        plt.colorbar(im0, ax=axes[row, 0], fraction=0.046, pad=0.04)

        im1 = axes[row, 1].imshow(pred_p, cmap='RdBu_r',
                                    vmin=vmin_p, vmax=vmax_p)
        ...
        im2 = axes[row, 2].imshow(err_p, cmap='RdBu_r',
        ...
        if is_main:
            ...
            if results.get('per_clip'):
                ...
                plot_loss_distribution(
                    results['per_clip'],
                    str(ckpt_dir / "loss_distribution_taylor_green_2d.png"),
                )
