...
This script loads the unfrozen Encoder/Decoder checkpoint.

Usage:
    python visualize_burgers_lora_v2.py --checkpoint checkpoints_burgers_lora_v2/best_lora_v2.pt --config configs/finetune_burgers_v2.yaml --output_dir ./vis_results
"""
...
def load_lora_v2_checkpoint(model, checkpoint_path: str):
    """Load V2 checkpoint (trainable_state_dict format)."""
    checkpoint = torch.load(checkpoint_path, map_location='cpu')

    if 'trainable_state_dict' in checkpoint:
        # V2 format: trainable_state_dict contains all trainable params
        ...
        # V1 format: only LoRA params
        lora_state_dict = checkpoint['lora_state_dict']
        model_state = model.model.transformer.state_dict()
        model_state.update(lora_state_dict)
        model.model.transformer.load_state_dict(model_state)
    else:
...
def load_lora_v2_model(config: dict, checkpoint_path: str, device: str = 'cuda'):
    """Load LoRA V2 model with unfrozen Encoder/Decoder."""
    ...
    print(f"Loading V2 checkpoint from: {checkpoint_path}")
    load_lora_v2_checkpoint(model, checkpoint_path)
...
@torch.no_grad()
def run_inference(model, batch: dict, device: str = 'cuda') -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Run inference."""
...
def main():
    parser = argparse.ArgumentParser(description="LoRA V2 Model Visualization for Burgers")
    parser.add_argument('--checkpoint', type=str, required=True, help='Path to V2 checkpoint')
    ...
    model = load_lora_v2_model(config, args.checkpoint, device=device)
    ...
    if args.scan_all:
        ...
        for sample_idx in range(n_val_samples):
            ...
            _, _, pde_loss, _, _ = compute_pde_residual(
                output=output, input_data=input_data, batch=batch, config=config, device=device,
            )
        ...
        print(f"  RMSE (u):  Mean={np.mean(all_rmse_u):.6f}")
        print(f"  RMSE (v):  Mean={np.mean(all_rmse_v):.6f}")
        print(f"{'='*60}")
        return
    ...
    output_filename = "visualization_burgers_lora_v2.png"
    print("Plotting visualization...")
    plot_results(results, str(output_dir / output_filename))

    print("\n" + "=" * 60)
    print("Visualization Complete (LoRA V2 - Unfrozen Enc/Dec)")
    print("=" * 60)
