"""
LoRA V3 Model Visualization for 2D Wave Equation.
...
Usage:
    ...
    torchrun --nproc_per_node=4 tools/visualize_wave_2d_lora.py \
        ...
        --checkpoint checkpoints_wave_2d_lora_v3/best_lora.pt --scan_all

    # Single-GPU visualization (plot only)
    python tools/visualize_wave_2d_lora.py \
        --config configs/finetune_wave_2d_v3.yaml \
        --checkpoint checkpoints_wave_2d_lora_v3/best_lora.pt --output_dir ./wave_2d_vis
"""
...
from finetune.pde_loss_verified import Wave2DPDELoss

# Wave 2D channel indices in 18-channel layout
CH_U = 3   # scalar[0] = u (displacement)
CH_W = 4   # scalar[1] = w (velocity = u_t)
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA V3 Visualization for Wave 2D")
...
def create_pde_loss_fn(config: dict) -> Wave2DPDELoss:
    physics = config.get('physics', {})
    return Wave2DPDELoss(
...
def compute_pde_loss_from_output(
    ...
    pde_loss_fn: Wave2DPDELoss,
    c: torch.Tensor,
) -> torch.Tensor:
    """Compute Wave 2D PDE residual loss."""
    with torch.autocast(device_type='cuda', enabled=False):
        t0_u = input_data[:, 0:1, :, :, CH_U].float()
        t0_w = input_data[:, 0:1, :, :, CH_W].float()

        out_u = output[:, :, :, :, CH_U].float()
        out_w = output[:, :, :, :, CH_W].float()

        u = torch.cat([t0_u, out_u], dim=1)
        w = torch.cat([t0_w, out_w], dim=1)

        total_loss, losses = pde_loss_fn(u, w, c.float())
    return total_loss
...
def scan_all_distributed(accelerator, model, val_loader, config, t_input):
    ...
    local_rmse_w = torch.full((max_batches,), float('nan'), device=accelerator.device)
    ...
    local_vrmse_w = torch.full((max_batches,), float('nan'), device=accelerator.device)
    local_nrmse_u = torch.full((max_batches,), float('nan'), device=accelerator.device)
    local_nrmse_w = torch.full((max_batches,), float('nan'), device=accelerator.device)
    ...
    for i, batch in enumerate(val_loader):
        ...
        nu = batch['nu'].to(device=accelerator.device)  # wave speed c
        ...
        rmse_w = torch.sqrt(torch.mean((output[..., CH_W] - target_data[..., CH_W]) ** 2) + 1e-8)

        vrmse_u = _vrmse_torch(target_data[..., CH_U], output[..., CH_U])
        vrmse_w = _vrmse_torch(target_data[..., CH_W], output[..., CH_W])

        nrmse_u = _nrmse_torch(target_data[..., CH_U], output[..., CH_U])
        nrmse_w = _nrmse_torch(target_data[..., CH_W], output[..., CH_W])
        ...
        vrmse_all = (vrmse_u + vrmse_w) / 2
        nrmse_all = (nrmse_u + nrmse_w) / 2

        local_pde[i] = pde_loss.detach()
        local_rmse_u[i] = rmse_u.detach()
        local_rmse_w[i] = rmse_w.detach()
        local_rmse[i] = rmse.detach()
        local_vrmse_u[i] = vrmse_u.detach()
        local_vrmse_w[i] = vrmse_w.detach()
        local_nrmse_u[i] = nrmse_u.detach()
        local_nrmse_w[i] = nrmse_w.detach()
    ...
    all_rmse_w = accelerator.gather(local_rmse_w)
    all_rmse = accelerator.gather(local_rmse)
    all_vrmse_u = accelerator.gather(local_vrmse_u)
    all_vrmse_w = accelerator.gather(local_vrmse_w)
    all_nrmse_u = accelerator.gather(local_nrmse_u)
    all_nrmse_w = accelerator.gather(local_nrmse_w)
    ...
    valid_rmse_w = all_rmse_w[valid_mask].cpu().numpy()
    valid_rmse = all_rmse[valid_mask].cpu().numpy()
    valid_vrmse_u = all_vrmse_u[valid_mask].cpu().numpy()
    valid_vrmse_w = all_vrmse_w[valid_mask].cpu().numpy()
    valid_nrmse_u = all_nrmse_u[valid_mask].cpu().numpy()
    valid_nrmse_w = all_nrmse_w[valid_mask].cpu().numpy()
    ...
    if n > 0:
        return {
            ...
            'rmse_w': float(np.mean(valid_rmse_w)),
            'rmse': float(np.mean(valid_rmse)),
            'vrmse_u': float(np.mean(valid_vrmse_u)),
            'vrmse_w': float(np.mean(valid_vrmse_w)),
            'nrmse_u': float(np.mean(valid_nrmse_u)),
            'nrmse_w': float(np.mean(valid_nrmse_w)),
            ...
            'per_clip': {
                ...
                'vrmse_w': valid_vrmse_w,
                'nrmse_u': valid_nrmse_u,
                'nrmse_w': valid_nrmse_w,
                'vrmse_all': valid_vrmse_all,
                'nrmse_all': valid_nrmse_all,
            },
        }
    return {'pde': 0, 'rmse_u': 0, 'rmse_w': 0, 'rmse': 0,
            'vrmse_u': 0, 'vrmse_w': 0, 'nrmse_u': 0, 'nrmse_w': 0,
...
def run_visualization(model, dataset, config, device, t_input, num_samples, seed, output_dir):
    ...
    for i, sample_idx in enumerate(sample_indices):
        ...
        nu = batch['nu'].to(device=device)  # wave speed c
        input_data = data[:, :t_input]
        ...
        rmse_w = torch.sqrt(torch.mean((output[..., CH_W] - target[..., CH_W]) ** 2) + 1e-8).item()

        vrmse_u = _vrmse_torch(target[..., CH_U], output[..., CH_U]).item()
        vrmse_w = _vrmse_torch(target[..., CH_W], output[..., CH_W]).item()

        nrmse_u = _nrmse_torch(target[..., CH_U], output[..., CH_U]).item()
        nrmse_w = _nrmse_torch(target[..., CH_W], output[..., CH_W]).item()
        ...
        res_data = {
            ...
            'gt_w': target[0, last, :, :, CH_W].float().cpu().numpy(),
            'pred_w': output[0, last, :, :, CH_W].float().cpu().numpy(),
            'sample_idx': sample_idx, 'start_t': start_t,
        }
        results.append(res_data)

        all_metrics['rmse_u'].append(rmse_u)
        all_metrics['rmse_w'].append(rmse_w)
        all_metrics['vrmse_u'].append(vrmse_u)
        all_metrics['vrmse_w'].append(vrmse_w)
        all_metrics['nrmse_u'].append(nrmse_u)
        all_metrics['nrmse_w'].append(nrmse_w)
        ...
        print(f"  Sample {sample_idx}: t_start={start_t}, "
              f"RMSE_u={rmse_u:.6f}, RMSE_w={rmse_w:.6f}, "
              f"nRMSE_u={nrmse_u:.6f}, nRMSE_w={nrmse_w:.6f}, PDE={pde_loss:.6f}")

    print("Plotting scalar visualization...")
    plot_scalar(results, str(output_dir / "visualization_wave_2d_scalar.png"))

    print(f"\n{'='*60}")
    print("Visualization Complete (Wave 2D LoRA)")
    print(f"{'='*60}")
    print(f"\nAverage Metrics ({len(results)} samples):")
    print(f"  - RMSE (u):        {np.mean(all_metrics['rmse_u']):.6f}")
    print(f"  - RMSE (w):        {np.mean(all_metrics['rmse_w']):.6f}")
    print(f"  - VRMSE (u):       {np.mean(all_metrics['vrmse_u']):.6f}")
    print(f"  - VRMSE (w):       {np.mean(all_metrics['vrmse_w']):.6f}")
    print(f"  - nRMSE (u):       {np.mean(all_metrics['nrmse_u']):.6f}")
    print(f"  - nRMSE (w):       {np.mean(all_metrics['nrmse_w']):.6f}")
...
def plot_scalar(results: list, save_path: str):
    """Plot GT vs Prediction vs Error for u and w."""
    ...
    for row, res in enumerate(results):
        ...
        im0 = axes[row, 0].imshow(gt_u, cmap='RdBu_r',
        ...
        im1 = axes[row, 1].imshow(pred_u, cmap='RdBu_r',
                                    vmin=vmin_u, vmax=vmax_u)
        ...
        im2 = axes[row, 2].imshow(err_u, cmap='RdBu_r',
                                    vmin=-err_u_max, vmax=err_u_max)
        axes[row, 2].set_title(f'Error u (MAE={np.mean(np.abs(err_u)):.4f})', fontsize=11)
        plt.colorbar(im2, ax=axes[row, 2], fraction=0.046, pad=0.04)

        # w (velocity)
        vmin_w = min(gt_w.min(), pred_w.min())
        vmax_w = max(gt_w.max(), pred_w.max())

        im3 = axes[row, 3].imshow(gt_w, cmap='RdBu_r',
                                    vmin=vmin_w, vmax=vmax_w)
        axes[row, 3].set_title('GT (w)', fontsize=11)
        plt.colorbar(im3, ax=axes[row, 3], fraction=0.046, pad=0.04)

        im4 = axes[row, 4].imshow(pred_w, cmap='RdBu_r',
                                    vmin=vmin_w, vmax=vmax_w)
        rmse_w = np.sqrt(np.mean(err_w**2))
        vrmse_w = _vrmse_np(gt_w, pred_w)
        axes[row, 4].set_title(f'Pred w (RMSE={rmse_w:.4f} VRMSE={vrmse_w:.4f})', fontsize=11)
        plt.colorbar(im4, ax=axes[row, 4], fraction=0.046, pad=0.04)

        err_w_max = max(np.percentile(np.abs(err_w), 95), 1e-6)
        im5 = axes[row, 5].imshow(err_w, cmap='RdBu_r',
                                    vmin=-err_w_max, vmax=err_w_max)
        axes[row, 5].set_title(f'Error w (MAE={np.mean(np.abs(err_w)):.4f})', fontsize=11)
...
def main():
    ...
    if is_main:
        print(f"{'='*60}")
        print(f"Wave 2D LoRA V3 Evaluation")
        print(f"{'='*60}")
    ...
    if args.scan_all:
        ...
        if is_main:
            ...
            print(f"  RMSE (w):      {results['rmse_w']:.6f}")
            print(f"  RMSE:          {results['rmse']:.6f}")
            print(f"  VRMSE (u):     {results['vrmse_u']:.6f}")
            print(f"  VRMSE (w):     {results['vrmse_w']:.6f}")
            print(f"  nRMSE (u):     {results['nrmse_u']:.6f}")
            print(f"  nRMSE (w):     {results['nrmse_w']:.6f}")
            ...
            if results.get('per_clip'):
                ...
                plot_loss_distribution(
                    results['per_clip'],
                    str(ckpt_dir / "loss_distribution_wave_2d.png"),
                )
