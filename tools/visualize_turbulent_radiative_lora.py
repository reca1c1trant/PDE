"""
LoRA V3 Model Visualization for Turbulent Radiative Layer (Compressible Euler).
...
Usage:
    ...
    python tools/visualize_turbulent_radiative_lora.py \
        --config configs/finetune_turbulent_radiative_v3.yaml \
        --checkpoint checkpoints_turbulent_radiative_lora_v3/best_lora.pt --output_dir ./turbulent_radiative_vis
"""
...
# TRL channel indices in 18-channel layout
CH_VX = 0       # vector: Vx
CH_VY = 1       # vector: Vy
CH_DENSITY = 7  # scalar[4] = density
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA V3 Visualization for Turbulent Radiative Layer")
...
def create_pde_loss_fn(config: dict) -> TurbulentRadiativePDELoss:
    physics = config.get('physics', {})
    return TurbulentRadiativePDELoss(
        nx=physics.get('nx', 128),
        ny=physics.get('ny', 384),
        Lx=physics.get('Lx', 1.0),
        Ly=physics.get('Ly', 3.0),
        dt=physics.get('dt', 1.597033),
        skip_bl=physics.get('skip_bl', 10),
    )
...
def compute_pde_loss_from_output(
    ...
    """Compute Turbulent Radiative PDE residual loss (compressible Euler)."""
    with torch.autocast(device_type='cuda', enabled=False):
        t0_vx = input_data[:, 0:1, :, :, CH_VX].float()
        t0_vy = input_data[:, 0:1, :, :, CH_VY].float()
        t0_rho = input_data[:, 0:1, :, :, CH_DENSITY].float()
        t0_P = input_data[:, 0:1, :, :, CH_PRESS].float()

        out_vx = output[:, :, :, :, CH_VX].float()
        out_vy = output[:, :, :, :, CH_VY].float()
        out_rho = output[:, :, :, :, CH_DENSITY].float()
        out_P = output[:, :, :, :, CH_PRESS].float()

        vx = torch.cat([t0_vx, out_vx], dim=1)
        vy = torch.cat([t0_vy, out_vy], dim=1)
        rho = torch.cat([t0_rho, out_rho], dim=1)
        P = torch.cat([t0_P, out_P], dim=1)

        total_loss, losses = pde_loss_fn(vx, vy, rho, P)
    return total_loss
...
def scan_all_distributed(accelerator, model, val_loader, config, t_input):
    ...
    local_rmse_density = torch.full((max_batches,), float('nan'), device=accelerator.device)
    ...
    local_vrmse_density = torch.full((max_batches,), float('nan'), device=accelerator.device)
    ...
    local_nrmse_density = torch.full((max_batches,), float('nan'), device=accelerator.device)
    ...
    for i, batch in enumerate(val_loader):
        ...
        rmse_density = torch.sqrt(torch.mean((output[..., CH_DENSITY] - target_data[..., CH_DENSITY]) ** 2) + 1e-8)
        ...
        vrmse_density = _vrmse_torch(target_data[..., CH_DENSITY], output[..., CH_DENSITY])
        ...
        nrmse_density = _nrmse_torch(target_data[..., CH_DENSITY], output[..., CH_DENSITY])
        ...
        local_rmse_density[i] = rmse_density.detach()
    ...
    all_rmse_density = accelerator.gather(local_rmse_density)
    ...
    all_vrmse_density = accelerator.gather(local_vrmse_density)
    ...
    all_nrmse_density = accelerator.gather(local_nrmse_density)
    ...
    valid_rmse_density = all_rmse_density[valid_mask].cpu().numpy()
    ...
    valid_vrmse_density = all_vrmse_density[valid_mask].cpu().numpy()
    ...
    valid_nrmse_density = all_nrmse_density[valid_mask].cpu().numpy()
    ...
    if n > 0:
        return {
            ...
            'rmse_density': float(np.mean(valid_rmse_density)),
...
def run_visualization(model, dataset, config, device, t_input, num_samples, seed, output_dir):
    ...
    results = []
    all_metrics = {'rmse_vx': [], 'rmse_vy': [], 'rmse_density': [], 'rmse_press': [],
                    'vrmse_vx': [], 'vrmse_vy': [], 'vrmse_density': [], 'vrmse_press': [],
                    'nrmse_vx': [], 'nrmse_vy': [], 'nrmse_density': [], 'nrmse_press': [],
                    'vrmse_all': [], 'nrmse_all': [],
                    'pde': []}
    ...
    for i, sample_idx in enumerate(sample_indices):
        ...
        rmse_density = torch.sqrt(torch.mean((output[..., CH_DENSITY] - target[..., CH_DENSITY]) ** 2) + 1e-8).item()
        ...
        vrmse_density = _vrmse_torch(target[..., CH_DENSITY], output[..., CH_DENSITY]).item()
        ...
        res_data = {
            ...
            'gt_density': target[0, last, :, :, CH_DENSITY].float().cpu().numpy(),
            'pred_density': output[0, last, :, :, CH_DENSITY].float().cpu().numpy(),
        ...
        print(f"  Sample {sample_idx}: t_start={start_t}, "
              ...
              f"RMSE_rho={rmse_density:.6f}, RMSE_p={rmse_press:.6f}, "
              f"VRMSE_vx={vrmse_vx:.6f}, VRMSE_vy={vrmse_vy:.6f}, "
              f"VRMSE_rho={vrmse_density:.6f}, VRMSE_p={vrmse_press:.6f}, PDE={pde_loss:.6f}")

    print("Plotting velocity visualization...")
    plot_velocity(results, str(output_dir / "visualization_turbulent_radiative_velocity.png"))

    print("Plotting scalar visualization...")
    plot_scalar(results, str(output_dir / "visualization_turbulent_radiative_scalar.png"))

    print(f"\n{'='*60}")
    print("Visualization Complete (Turbulent Radiative LoRA)")
    print(f"{'='*60}")
    ...
    print(f"  - RMSE (density):  {np.mean(all_metrics['rmse_density']):.6f}")
    ...
    print(f"  - VRMSE (density): {np.mean(all_metrics['vrmse_density']):.6f}")
    ...
    print(f"  - nRMSE (density): {np.mean(all_metrics['nrmse_density']):.6f}")
...
def plot_scalar(results: list, save_path: str):
    """Plot GT vs Prediction vs Error for density and pressure."""
    n_samples = len(results)
    fig, axes = plt.subplots(n_samples, 6, figsize=(30, 4 * n_samples))
    ...
    for row, res in enumerate(results):
        gt_rho = res['gt_density']
        pred_rho = res['pred_density']
        err_rho = pred_rho - gt_rho
        gt_p = res['gt_press']
        ...
        # Density
        vmin_rho = min(gt_rho.min(), pred_rho.min())
        vmax_rho = max(gt_rho.max(), pred_rho.max())

        im0 = axes[row, 0].imshow(gt_rho, cmap='RdBu_r',
                                    vmin=vmin_rho, vmax=vmax_rho)
        axes[row, 0].set_title('GT (density)', fontsize=11)
        axes[row, 0].set_ylabel(f'Sample {sample_idx}\nt0={start_t}', fontsize=10)
        plt.colorbar(im0, ax=axes[row, 0], fraction=0.046, pad=0.04)

        im1 = axes[row, 1].imshow(pred_rho, cmap='RdBu_r',
                                    vmin=vmin_rho, vmax=vmax_rho)
        rmse_rho = np.sqrt(np.mean(err_rho**2))
        vrmse_rho = _vrmse_np(gt_rho, pred_rho)
        axes[row, 1].set_title(f'Pred density (RMSE={rmse_rho:.4f} VRMSE={vrmse_rho:.4f})', fontsize=11)
        plt.colorbar(im1, ax=axes[row, 1], fraction=0.046, pad=0.04)

        err_rho_max = max(np.percentile(np.abs(err_rho), 95), 1e-6)
        im2 = axes[row, 2].imshow(err_rho, cmap='RdBu_r',
                                    vmin=-err_rho_max, vmax=err_rho_max)
        axes[row, 2].set_title(f'Error density (MAE={np.mean(np.abs(err_rho)):.4f})', fontsize=11)
        plt.colorbar(im2, ax=axes[row, 2], fraction=0.046, pad=0.04)
...
def main():
    ...
    if is_main:
        print(f"{'='*60}")
        print(f"Turbulent Radiative LoRA V3 Evaluation")
        print(f"{'='*60}")
        ...
        if is_main:
            ...
            if results.get('per_clip'):
                ...
                plot_loss_distribution(
                    results['per_clip'],
                    str(ckpt_dir / "loss_distribution_turbulent_radiative.png"),
                )
