...
from finetune.model_lora_muon import PDELoRAModelMuon, load_lora_checkpoint
...
def load_model(config: dict, checkpoint_path: str, is_main: bool = True):
    ...
    # Load enc/dec + LoRA from init_from (PEFT key mapping)
...
def main():
    ...
    if is_main:
        print(f"{'='*60}")
        print(f"Split-NS Eval — {pde_type}")
        print(f"{'='*60}")
