"""
LoRA V3 Model Visualization for KP-II (Kadomtsev-Petviashvili) 2D.

Multi-GPU evaluation using Accelerate (same pattern as training).

Usage:
    # Multi-GPU full evaluation
    torchrun --nproc_per_node=4 tools/visualize_kp2_2d_lora.py \
        --config configs/finetune_kp2_2d_v3.yaml \
        --checkpoint checkpoints_kp2_2d_lora_v3/best_lora.pt --scan_all

    # Single-GPU visualization (plot only)
    python tools/visualize_kp2_2d_lora.py \
        --config configs/finetune_kp2_2d_v3.yaml \
        --checkpoint checkpoints_kp2_2d_lora_v3/best_lora.pt --output_dir ./kp2_2d_vis
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA V3 Visualization for KP-II 2D")
...
def create_pde_loss_fn(config: dict) -> KP2DPDELoss:
...
def run_visualization(model, dataset, config, device, t_input, num_samples, seed, output_dir):
    ...
    plot_scalar(results, str(output_dir / "visualization_kp2_2d_scalar.png"))

    print(f"\n{'='*60}")
    print("Visualization Complete (KP-II 2D LoRA)")
    print(f"{'='*60}")
...
def main():
    ...
    if is_main:
        print(f"{'='*60}")
        print(f"KP-II 2D LoRA V3 Evaluation")
        print(f"{'='*60}")
        ...
        if is_main:
            ...
            if results.get('per_clip'):
                ...
                plot_loss_distribution(
                    results['per_clip'],
                    str(ckpt_dir / "loss_distribution_kp2_2d.png"),
                )
