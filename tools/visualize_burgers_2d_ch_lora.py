"""
LoRA V3 Model Visualization for 2D Burgers (Cole-Hopf, periodic).
...
Usage:
    ...
    torchrun --nproc_per_node=4 tools/visualize_burgers_2d_ch_lora.py \
        ...
        --checkpoint checkpoints_burgers_2d_ch_lora_v3/best_lora.pt --scan_all

    # Single-GPU visualization (plot only)
    python tools/visualize_burgers_2d_ch_lora.py \
        --config configs/finetune_burgers_2d_ch_v3.yaml \
        --checkpoint checkpoints_burgers_2d_ch_lora_v3/best_lora.pt --output_dir ./burgers_2d_ch_vis
"""
...
from finetune.pde_loss_verified import Burgers2DCHPDELoss

# Burgers 2D channel indices in 18-channel layout
CH_VX = 0       # vector: Vx
CH_VY = 1       # vector: Vy
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA V3 Visualization for 2D Burgers (Cole-Hopf)")
...
def create_pde_loss_fn(config: dict) -> Burgers2DCHPDELoss:
    physics = config.get('physics', {})
    return Burgers2DCHPDELoss(
...
def compute_pde_loss_from_output(
    ...
    pde_loss_fn: Burgers2DCHPDELoss,
    nu: torch.Tensor,
) -> torch.Tensor:
    """Compute Burgers 2D PDE residual loss."""
    with torch.autocast(device_type='cuda', enabled=False):
        ...
        total_loss, losses = pde_loss_fn(u, v, nu=nu.float())
    return total_loss
...
def scan_all_distributed(accelerator, model, val_loader, config, t_input):
    ...
    for i, batch in enumerate(val_loader):
        ...
        vrmse_all = (vrmse_vx + vrmse_vy) / 2
        nrmse_all = (nrmse_vx + nrmse_vy) / 2
    ...
    return {'pde': 0, 'rmse_vx': 0, 'rmse_vy': 0, 'rmse': 0,
            'vrmse_vx': 0, 'vrmse_vy': 0, 'nrmse_vx': 0, 'nrmse_vy': 0,
...
def run_visualization(model, dataset, config, device, t_input, num_samples, seed, output_dir):
    ...
    for i, sample_idx in enumerate(sample_indices):
        ...
        print(f"  Sample {sample_idx}: t_start={start_t}, "
              ...
              f"nRMSE_vx={nrmse_vx:.6f}, nRMSE_vy={nrmse_vy:.6f}, PDE={pde_loss:.6f}")

    print("Plotting velocity visualization...")
    plot_velocity(results, str(output_dir / "visualization_burgers_2d_ch_velocity.png"))

    print(f"\n{'='*60}")
    print("Visualization Complete (Burgers 2D Cole-Hopf LoRA)")
    print(f"{'='*60}")
...
def main():
    ...
    if is_main:
        print(f"{'='*60}")
        print(f"Burgers 2D Cole-Hopf LoRA V3 Evaluation")
        print(f"{'='*60}")
        ...
        if is_main:
            ...
            if results.get('per_clip'):
                ...
                plot_loss_distribution(
                    results['per_clip'],
                    str(ckpt_dir / "loss_distribution_burgers_2d_ch.png"),
                )
