...
def compute_rmse_loss(output, target):
    ...
    For Burgers equation, we use channels 0 (u) and 1 (v).
    ...
    Returns:
        rmse_loss: RMSE loss (u and v channels)
    """
    ...
    with torch.autocast(device_type='cuda', enabled=False):
        ...
        # Use first 2 channels (u, v) for Burgers
        output_uv = output[..., :2]
        target_uv = target[..., :2]

        mse = torch.mean((output_uv - target_uv) ** 2)
...
def compute_boundary_loss(output, target):
    ...
    Returns:
        boundary_rmse: Scalar tensor (u and v channels)
    """
    # Disable autocast for numerical stability
    with torch.autocast(device_type='cuda', enabled=False):
        output = output.float()
        ...
        left_pred = output_uv[:, :, 1:-1, 0, :]
        left_target = target_uv[:, :, 1:-1, 0, :]

        # Right edge (excluding corners)
        right_pred = output_uv[:, :, 1:-1, -1, :]
        right_target = target_uv[:, :, 1:-1, -1, :]

        # Bottom edge (excluding corners)
        bottom_pred = output_uv[:, :, 0, 1:-1, :]
        bottom_target = target_uv[:, :, 0, 1:-1, :]

        # Top edge (excluding corners)
        top_pred = output_uv[:, :, -1, 1:-1, :]
        top_target = target_uv[:, :, -1, 1:-1, :]
        ...
        all_target = torch.cat([
            left_target.flatten(),
            right_target.flatten(),
            bottom_target.flatten(),
            top_target.flatten()
        ])
