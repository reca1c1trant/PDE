"""
SUT-FNO V2 Training Script.

Train Spectral U-Net Transformer + FNO V2 model on Burgers equation with:
- PDE residual loss (physics-informed)
- RMSE loss (supervised)
- **NEW: Gradient-Aware Loss (spatial derivative supervision)**

V2 Changes:
- Uses model_sut_fno_v2 with Learnable Spectral Attention
- Added Gradient-Aware Loss for better PDE derivative accuracy

Usage:
    # Single GPU
    python train_sut_fno_v2.py --config configs/sut_fno_burgers_v2.yaml

    # Multi-GPU
    torchrun --nproc_per_node=8 train_sut_fno_v2.py --config configs/sut_fno_burgers_v2.yaml
"""
...
from model_sut_fno_v2 import create_sut_fno_model_v2
...
def compute_pde_loss(output, input_data, batch, config, accelerator):
    ...
    # Add t0 frame for PDE computation
...
def compute_rmse_loss(output, target):
    ...
    # Only compute on channels 0, 1 (u, v)
    pred_uv = output[..., :2]
    target_uv = target[..., :2]
...
def compute_gradient_loss(output: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    """
    Compute Gradient-Aware Loss for spatial derivatives.

    This loss directly supervises the spatial gradients (du/dx, du/dy, dv/dx, dv/dy),
    which are critical for PDE accuracy.

    Args:
        output: [B, T, H, W, C] predicted tensor
        target: [B, T, H, W, C] ground truth tensor

    Returns:
        grad_loss: scalar loss for spatial gradients
    """
    # Only compute on channels 0, 1 (u, v)
    pred_uv = output[..., :2].float()    # [B, T, H, W, 2]
    target_uv = target[..., :2].float()  # [B, T, H, W, 2]

    # Compute spatial gradients using finite differences
    # dx gradients (along W dimension, axis=-2 in [B,T,H,W,C])
    pred_dx = pred_uv[:, :, :, 1:, :] - pred_uv[:, :, :, :-1, :]
    target_dx = target_uv[:, :, :, 1:, :] - target_uv[:, :, :, :-1, :]

    # dy gradients (along H dimension, axis=-3 in [B,T,H,W,C])
...
def validate(model, val_loader, config, accelerator):
    ...
    total_grad_loss = torch.zeros(1, device=accelerator.device)
    ...
    for batch in val_loader:
        ...
        grad_loss = compute_gradient_loss(output, target)

        total_pde_loss += pde_loss.detach()
        total_rmse_loss += rmse_loss.detach()
        total_grad_loss += grad_loss.detach()
        num_batches += 1
    ...
    total_grad_loss = accelerator.reduce(total_grad_loss, reduction='sum')
    ...
    return (
        ...
        (total_grad_loss / n).item() if n > 0 else 0,
    )
...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/sut_fno_burgers_v2.yaml')
    ...
    lambda_grad = config['training'].get('lambda_grad', 0.1)  # NEW: Gradient loss weight
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info(f"SUT-FNO V2 Training - Burgers Equation")
        logger.info(f"{'='*60}")
        logger.info(f"Config: {args.config}")
        logger.info(f"Max Epochs: {max_epochs}")
        logger.info(f"Warmup Steps: {warmup_steps}")
        logger.info(f"Learning Rate: {config['training']['learning_rate']}")
        logger.info(f"Loss: lambda_pde={lambda_pde}, lambda_rmse={lambda_rmse}, lambda_grad={lambda_grad}")
    ...
    # Create model V2
    model = create_sut_fno_model_v2(config)

    # Log parameter counts
    if accelerator.is_main_process:
        params = model.count_parameters()
        print(f"\n{'='*60}")
        print(f"SUT-FNO V2 Model Parameters (with Spectral Attention)")
        print(f"{'='*60}")
    ...
    # Init WandB
    if accelerator.is_main_process:
        run_name = f"sut-fno-v2-pde{lambda_pde}-rmse{lambda_rmse}-grad{lambda_grad}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["sut-fno-v2", "burgers", "spectral-attention", "gradient-loss"],
            }}
        )
    ...
    with Progress(
        ...
        train_task = progress.add_task("Training V2", total=total_steps)
        ...
        for epoch in range(max_epochs):
            ...
            for batch in train_loader:
                ...
                # Use float32 for FFT operations
                data = batch['data'].to(device=accelerator.device, dtype=torch.float32)
                input_data = data[:, :-1]
                target = data[:, 1:]
                ...
                grad_loss_reduced = accelerator.reduce(grad_loss.detach(), reduction='mean')

                epoch_pde_loss += pde_loss_reduced.item()
                epoch_rmse_loss += rmse_loss_reduced.item()
                epoch_grad_loss += grad_loss_reduced.item()

                # Update progress
                phase_str = "[warmup]" if in_warmup else f"[E{epoch+1}]"
                desc = f"{phase_str} PDE={pde_loss_reduced.item():.4f} RMSE={rmse_loss_reduced.item():.4f} Grad={grad_loss_reduced.item():.4f}"
                ...
                if global_step % log_interval == 0:
                    log_dict = {
                        ...
                        'train/grad_loss': grad_loss_reduced.item(),
                ...
                if global_step % eval_interval == 0:
                    accelerator.wait_for_everyone()
                    val_pde, val_rmse, val_grad = validate(model, val_loader, config, accelerator)

                    accelerator.log({
                        'val/pde_loss': val_pde,
                        'val/rmse_loss': val_rmse,
                        'val/grad_loss': val_grad,
                        'val/total_loss': lambda_pde * val_pde + lambda_rmse * val_rmse + lambda_grad * val_grad,
                    }, step=global_step)

                    if accelerator.is_main_process:
                        console.print(
                            f"\n[green]Step {global_step}/{total_steps} {phase_str}:[/green] "
                            f"val_pde={val_pde:.6f}, val_rmse={val_rmse:.6f}, val_grad={val_grad:.6f}"
                        )

                    # Best model selection
                    val_loss = lambda_pde * val_pde + lambda_rmse * val_rmse + lambda_grad * val_grad
                        ...
                        if val_loss < best_val_loss:
                            ...
                            if accelerator.is_main_process:
                                ...
                                save_checkpoint(
                                    ...
                                    scheduler=scheduler,
                                    global_step=global_step,
                                    metrics={'pde_loss': val_pde, 'rmse_loss': val_rmse, 'grad_loss': val_grad},
            ...
            if epoch_steps > 0:
                ...
                avg_grad = epoch_grad_loss / epoch_steps
                accelerator.log({
                    'epoch/train_pde_loss': avg_pde,
                    'epoch/train_rmse_loss': avg_rmse,
                    'epoch/train_grad_loss': avg_grad,
                    'epoch': epoch + 1
                }, step=global_step)
                ...
                if accelerator.is_main_process:
                    console.print(
                        ...
                        f"avg_pde={avg_pde:.6f}, avg_rmse={avg_rmse:.6f}, avg_grad={avg_grad:.6f}"
                    )

    accelerator.end_training()

    if accelerator.is_main_process:
        table = Table(title="SUT-FNO V2 Training Complete", show_header=False, border_style="green")
