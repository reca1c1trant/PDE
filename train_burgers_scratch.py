"""
Burgers2D Training from Scratch.

Train PDE model from scratch on Burgers equation with:
- PDE residual loss (physics-informed)
- RMSE loss (supervised, optional)

Usage:
    # Single GPU
    python train_burgers_scratch.py --config configs/burgers_scratch.yaml

    # Multi-GPU
    torchrun --nproc_per_node=8 train_burgers_scratch.py --config configs/burgers_scratch.yaml
"""
...
def create_model(config: dict) -> nn.Module:
    """Create PDECausalModel from scratch (no pretrained weights)."""
    ...
    if IS_MAIN_PROCESS:
        ...
        print(f"\n{'='*60}")
        print(f"Model Parameters (Training from Scratch)")
        print(f"{'='*60}")
        print(f"Encoder:     {enc_total:>12,}")
        print(f"Decoder:     {dec_total:>12,}")
        print(f"Transformer: {trans_total:>12,}")
        print(f"{'-'*60}")
...
def compute_rmse_loss(output, target):
    ...
    Returns:
        rmse_loss: RMSE loss (only on valid channels u, v)
    """
...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/burgers_scratch.yaml')
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info(f"Burgers2D Training from Scratch")
        logger.info(f"{'='*60}")
    ...
    # Init WandB
    if accelerator.is_main_process:
        run_name = f"burgers-scratch-pde{lambda_pde}-rmse{lambda_rmse}"
    ...
    with Progress(
        ...
        for epoch in range(max_epochs):
            ...
            for batch in train_loader:
                ...
                desc = f"{phase_str} PDE={pde_loss_reduced.item():.4f}"
                if lambda_rmse > 0:
                ...
                if global_step % eval_interval == 0:
                    ...
                    if accelerator.is_main_process:
                        console.print(
                            ...
                            f"val_pde={val_pde:.6f}, val_rmse={val_rmse:.6f}, constraint={val_constraint:.6f}"
                        )
                        ...
                        if val_loss < best_val_loss:
                            ...
                            if accelerator.is_main_process:
                                ...
                                save_checkpoint(
                                    ...
                                    scheduler=scheduler,
                                    global_step=global_step,
                                    metrics={'pde_loss': val_pde, 'rmse_loss': val_rmse, 'constraint': val_constraint},
            ...
            if epoch_steps > 0:
                ...
                if accelerator.is_main_process:
                    console.print(f"\n[blue]Epoch {epoch+1}/{max_epochs} completed:[/blue] avg_pde={avg_pde:.6f}")
